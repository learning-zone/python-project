...
{
	self.opener.document.MyFrm.username.value = Code;
	self.opener.document.MyFrm.action = "LogReportView.php";
	window.close();
}
//-->
</SCRIPT>
...
<BODY CLASS="bodyline">
<FORM NAME="searchfrm" METHOD="POST" ACTION="usersearch4.php">
...
if (rowcount($rs) == 0)
{
	$query = "SELECT * FROM student_m WHERE Activated='On' AND username LIKE '$username%' ORDER BY username";
	$rs = execute($query) or die("QUERY $query " . error_description());
	if(rowcount($rs)==0)
	{
		echo "<TR>";
...
<FORM name="frmtemp" action="usersearch4.php" mehtod="POST">
