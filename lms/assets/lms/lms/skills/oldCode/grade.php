...
$sql = execute("SELECT * FROM users WHERE username='$user'");
$rs=fetcharray($sql);
$UserId=$rs[id];
?>
...
{	?>
    <a href="grade.php?action=mod"><input type="button" value="Modify" class="bgbutton"></a><br>
    <?php
}
if($action=='mod' and $stu_id=='') 
{
	
	if($_GET['deleteid'] and $_GET['delete'])
	{
		if($_GET['delete']=='yes')
		{
			mysql_query("UPDATE `exam_m` SET `sts` = '0' WHERE `id` = '$_GET[deleteid]'");
			?>
			<SCRIPT LANGUAGE="JavaScript">
		...
		if($_GET['delete']=='no')
		{
			mysql_query("UPDATE `exam_m` SET `sts` = '1' WHERE `id` = '$_GET[deleteid]'");
			?>
			<SCRIPT LANGUAGE="JavaScript">
	...
	?>
    <a href="grade.php"><input type="button" value="Add New" class="bgbutton"></a><br>
    <?php
	$sytemdate=date("Y-m-d");
	?>
    <table  class='forumline' align='center' width="60%" border="1" cellspacing="0" cellpadding="0">
  <tr>
	<td colspan="6" class="head" align="center">Modify Examination </td>
	</tr>
  <tr>
  ...
  <?
	$inc=1;
	$temsql3=mysql_query("select a.id , a.descr , a.f_date , a.t_date , b.year_name from exam_m a, course_year b where sts=1 and b.year_id=a.class order by a.class");
	while($r=mysql_fetch_array($temsql3))
	{
		echo "<tr height='25'>
    <td align='center'>$inc</td>
    <td nowrap>&nbsp;&nbsp;<a href='grade.php?stu_id=$r[id]'>$r[descr]</a></td>
    <td nowrap>&nbsp;&nbsp;$r[year_name]</td>
	<td align='center' nowrap>$r[f_date]</td>
    <td align='center' nowrap>$r[t_date]</td>
    <td nowrap>&nbsp;&nbsp;<a href='grade.php?action=mod&delete=yes&deleteid=$r[id]'>DELETE</a></td>
  </tr>";
  $inc++;
	}
   ...
   <table  class='forumline' align='center' width="60%" border="1" cellspacing="0" cellpadding="0">
  <tr>
	<td colspan="6" class="head" align="center">Activate Examination </td>
	</tr>
  <tr>
    <td width="10%" align="center" class="rowpic">Sl.No.</td>
    <td align="center" class="rowpic">Exam Name</td>
  ...
  <?
	$inc=1;
	$temsql3=mysql_query("select a.id , a.descr , a.f_date , a.t_date , b.year_name from exam_m a, course_year b where sts=0 and b.year_id=a.class order by a.class");
	while($r=mysql_fetch_array($temsql3))
	{
		echo "<tr height='25'>
    <td align='center'>$inc</td>
    <td nowrap>&nbsp;&nbsp;$r[descr]</td>
    <td nowrap>&nbsp;&nbsp;$r[year_name]</td>
	<td align='center' nowrap>$r[f_date]</td>
    <td align='center' nowrap>$r[t_date]</td>
    <td nowrap>&nbsp;&nbsp;<a href='grade.php?action=mod&delete=no&deleteid=$r[id]'>Activate</a></td>
  </tr>";
  $inc++;
	}
	...
	if($descr!='')
{
	...
	mysql_query("update exam_m set descr='$descr',sub_id='$subidstr',f_date='$fdate' , t_date='$tdate', exam_name='$ename', max_mark='$mmks',vct='$vct',sts=1 where id='$stu_id'");
	$iii=3;
	for($ii=0;$ii<sizeof($subid);$ii++)
	{
		...
		if(mysql_num_rows(mysql_query("select id from exam_timetable_m where exam_id='$stu_id' and subject_id='$subid[$ii]'")))
		{
			mysql_query("update exam_timetable_m set exam_date='$datename', exam_time='$timename' where exam_id='$stu_id' and subject_id='$subid[$ii]'");
			
		}
		else
		{
			mysql_query("insert into exam_timetable_m(exam_id,subject_id,exam_date,exam_time) values('$stu_id','$subid[$ii]','$datename','$timename')");
		}
		$iii++;
	}
	$course='';
...
{
		?>
    <a href="grade.php"><input type="button" value="Add New" class="bgbutton"></a></a>
    <?php
...
?>
	...
	$r1=mysql_fetch_array($rs_course);
	echo $r1[0];
	?></td></tr>
	<tr height='25'>
	...
	if($FromYear!='' and $course!='')
	{
	   ...
	   <tr><td class="head" colspan="5" align="center">Apply Subjects</td></tr>
		<tr><td align='center' width="50%" class="rowpic" nowrap>Subject Name</td><td align='center' width="30%" class="rowpic" nowrap>MAX.M</td><td align='center' class="rowpic" nowrap>Select</td>
		<?php
	  ...
	  while($rs1=mysql_fetch_array($suqry))
	  {
		  ...
		  echo "<td align='center'><input type='text' name='maxmark_$rs1[0]' id='maxm_$ssno' value='$mks' size='5' maxlength='3' onchange='selectMe1($ssno)'></td>";
		 
		  $dateold=mysql_fetch_row(mysql_query("select exam_date from exam_timetable_m where exam_id='$stu_id' and subject_id='$rs1[0]' "));
		  $timeold=mysql_fetch_row(mysql_query("select exam_time from exam_timetable_m where exam_id='$stu_id' and subject_id='$rs1[0]'"));
		  ...
		  <?php
		   ...
		   echo "<td align='center'><input type='checkbox' name='subid[]' id='mm_$ssno' value='$rs1[0]' $check ></td></tr>";
			$check='';
			$mks='';
			$ssno++;
			$iii++;
...
if($descr!='')
{
	...
	$ttdate=explode('/',$bdate);
	$tdate=$ttdate[2]."-".$ttdate[1]."-".$ttdate[0];
	$temsql=mysql_query("select * from exam_m where class='$FromYear' and f_date='$fdate' and t_date='$tdate' and exam_name='$ename'");
	$accyear=$_SESSION['AcademicYear'];
	$temsql12=mysql_query("select max(exam_count) from exam_m where curriculam='$course' and class='$FromYear' and accyear='$accyear'");
	$examid=mysql_fetch_row($temsql12);
	...
	else
	{
		...
		mysql_query("update exam_m set sts=1 where class='$FromYear' ");
		mysql_query("insert into exam_m (curriculam,class,f_date ,t_date,exam_name,max_mark,sub_id,exam_count,accyear,descr,vct,sts) values('$course','$FromYear','$fdate','$tdate','$ename','$mmks','$subidstr','$examidcount','$accyear','$descr','$vct',1)");
		$examiddet=mysql_fetch_row(mysql_query("select max(id) from exam_m "));
		$iii=3;
		for($ii=0;$ii<sizeof($subid);$ii++)
		{
			$datename=$_POST['date'.$iii];
...
if($action=='' and $stu_id=='') 
{
	...
	if($FromYear!='' and $course!='')
	{
	  ...
	  while($rs1=mysql_fetch_array($suqry))
	  {
			...
			<input type='text' name='maxmark_$rs1[0]' id='max_$sno' value='' size='5' maxlength='3' width='3' onchange='selectMe($sno)'></td>";
			 $datename="date".$iii;
		  $calandername="Calendar".$iii;
			?>
			...
			echo "<td align='center'><input type='checkbox' name='subid[]' id='mm_$sno' value='$rs1[0]' ></td></tr>";
			$sno++;
			$iii++;
	  }
	  ?></table>
