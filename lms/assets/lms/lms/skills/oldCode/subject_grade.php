...
{	
	...
	else
	{
		...
		//$yearstt=mysql_query("SELECT count(id)FROM `exam_grade_point` where`acc_year`='$a_year' and `sem`='$sem' and exam_id='$examname_m' and subject_id='$subject' and (from_point='$From' or to_point='$To')");
		//$yearstt1=mysql_fetch_row($yearstt);
		if($yearstt1[0]>0)
		{
		...
		else
		{
			if($sem!=0 and $From!='' and $To!='' and $grade!='' and $examname_m!='' and $subject!='')
			{
				...
				mysql_query("INSERT INTO `exam_grade_point` ( `from_point`, `to_point`, `tot_point`, `acc_year`, `sem`, `status`, `desc`,`exam_id`,`subject_id`) VALUES ( '$From', '$To', '$grade','$a_year','$sem','1','".addslashes($remark)."','$examname_m','$subject')");
				?>
...
if(isset($_POST['modify']))
{
	...
	for($i=0;$i<sizeof($cid);$i++)
	{
		...
		mysql_query("update exam_grade_point set `from_point`='$From1', `to_point`='$To1', `desc`='".addslashes($remark1)."',`tot_point`='$grade1' where id='$cid[$i]'");	
	}
		?>
		<Script language="JavaScript">
		alert("Updated successfully");
...
$sql4=mysql_query("select sub_id from exam_m where id='$examname_m'");
$subid=mysql_fetch_row($sql4);
...
<?php
	...
	{
		$sql4=mysql_query("select subject_name from subject_m where subject_id='$sub_id1[$j]'");
		$subdis=mysql_fetch_row($sql4);
...
<?php
		...
		$yearstt=mysql_query("SELECT id FROM `exam_grade_point` where`acc_year`='$a_year' and `sem`='$sem' and exam_id='$examname_m' and subject_id='$subject'");
		$yearstt2=mysql_fetch_row($yearstt);
		$yearstt2[0];
		if($yearstt2[0]==0)
		die();
?><table align='center' class='forumline' width='70%' border="1" >
...
<?php 
$yearstt=mysql_query("SELECT *FROM `exam_grade_point` where`acc_year`='$a_year' and `sem`='$sem' and exam_id='$examname_m' and subject_id='$subject'");
while($yearstt1=mysql_fetch_array($yearstt))
{
?>
	<tr>
