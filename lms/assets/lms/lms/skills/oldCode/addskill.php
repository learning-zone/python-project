...

{
...

	{
		...
		$Sql66=mysql_query(" select id from skill_grade_desc where student_id='$studentid' and acc_year='$accyeardet' and sub='$newid' and exam_id='$examid'");
...

		{
			...
			mysql_query("INSERT INTO skill_grade_desc (`class`, `sec`, `student_id`, `acc_year`, `desc1`, `exam_id`, `sub`) VALUES ( '$sem', '$class_section_id', '$studentid', '$accyeardet', '$desc1', '$examid', '$newid')");
...

	{
		...
		$Sql6=mysql_query(" select id from skill_grade where student_id='$studentid' and skill='$idin' and acc_year='$accyeardet'");

		if(mysql_num_rows($Sql6)>0)

		{

			mysql_query("update skill_grade set eval1='$eval1', eval2='$eval2' where student_id='$studentid' and skill='$idin' and acc_year='$accyeardet'");
...

		{
			...
			mysql_query("INSERT INTO skill_grade (id, divi, class, sec, student_id, skill, acc_year, eval1, eval2) VALUES (NULL, '$course', '$sem', '$class_section_id', '$studentid', '$idin', '$accyeardet', '$eval1', '$eval2')");
...

{
	...
	$sql1=mysql_query("SELECT a.subject_id , a.subject_name,a.elective FROM subject_m a, master_skills b where a.course_id='$course' and  a.course_year_id='$sem' and b.exam_id='$examid' and b.sub=a.subject_id and a.subject_id='$examidsub1[$g]'  group by b.sub ");
	...
	if($r2[2]=='Y')
	{
		...
		$studentstatus=mysql_fetch_row(mysql_query("select id from student_course where stu_id='$studentid' and acc_year='$accyeardet' and sub='$r2[0]'"));
		if(!$studentstatus)
		$flag1=0;
	}
		$flag2=1;
	...
	if($flag1)
	{
	   ...
	   $grade=0;
		...
		$sql2=mysql_query("SELECT id , skill,mark FROM master_skills where class='$sem' and sub='$examidsub1[$g]' and exam_id='$examid' order by posi");
	...
	echo "</td><input type='hidden' name='subarr[]' value='$r3[0]'>
							...
							$flagk=1;
							$elsectiveid=mysql_fetch_row(mysql_query("SELECT elective FROM subject_m where subject_id='$examidsub1[$g]'"));
							if($elsectiveid[0]=='Y')
							{							
