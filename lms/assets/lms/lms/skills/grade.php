...

{
...

	{
...

		{
			...
			execute("UPDATE `exam_m` SET `sts` = '0' WHERE `id` = '$_GET[deleteid]'");
...

		{
			...
			execute("UPDATE `exam_m` SET `sts` = '1' WHERE `id` = '$_GET[deleteid]'");
  ...
  <?
	...
	$temsql3=execute("select a.id , a.descr , a.f_date , a.t_date , b.year_name from exam_m a, course_year b where sts=1 and b.year_id=a.class order by a.class");
...

{
	...
	execute("update exam_m set descr='$descr',sub_id='$subidstr',f_date='$fdate' , t_date='$tdate', exam_name='$ename', max_mark='$mmks',vct='$vct',sts=1 where id='$stu_id'");
...

{
...

	{
		...
		execute("insert into exam_m (curriculam,class,f_date ,t_date,exam_name,max_mark,sub_id,exam_count,accyear,descr,vct,sts) values('$course','$FromYear','$fdate','$tdate','$ename','$mmks','$subidstr','$examidcount','$accyear','$descr','$vct',1)");
