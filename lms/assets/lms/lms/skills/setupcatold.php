...
<Script language="JavaScript">
  ...
  function RefreshMe()
  {
	  document.frm.action="setupcatold.php";
	  document.frm.submit();
  }
  function Summary()
  {
	  document.frm.action="setupcatold.php?Type=summary";
	  document.frm.submit();
  }
</script>
<Script language="JavaScript">
  ...
  <table align='center' class='forumline' width='100%'>
	...
	?>
      <td>&nbsp;&nbsp;
		...
		{
			$sectname=fetchrow(execute("SELECT codename,section_name FROM `class_section` WHERE id='$r2[2]' and status=1"));
      ...
      <?
		   $qResult = execute ("SELECT * FROM `grade_avg` WHERE `subject` = '$subject' AND `status` ='1'");
			$itms=fetcharray($qResult);
				...
				echo "SELECT a.id,a.student_id,a.first_name,a.last_name,a.admission_id,a.course_yearsem FROM student_m a,student_course b WHERE b.stu_id=a.id AND b.sub_sec='$subject' AND b.acc_year='$a_year' group by b.stu_id ORDER BY a.first_name";
				 $sql=execute("SELECT a.id,a.student_id,a.first_name,a.last_name,a.admission_id,a.course_yearsem FROM student_m a,student_course b WHERE b.stu_id=a.id AND b.sub_sec='$subject' AND b.acc_year='$a_year' group by b.stu_id ORDER BY a.first_name");	
      ...
      <tr>
       ...
       //++++++++++++++++++++++++++++++++++++++++++   TO FETCH EXSISTING COLUMN NAME  +++++++++++++++++++++++++++++++++++++++++++
		...
		while($rCol=fetcharray($resultCol))      
		{
		  ...
		  ?>
            <td nowrap align="center" width="5">ffffff
