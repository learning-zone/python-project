	  ...
	  while($rIns=fetcharray($sqlIns))
	  {
		 ...
		 if($student_id!='')
		 {
		    ...
		    {			
				...
				$id=fetcharray(execute("SELECT id FROM `$tablename` ORDER BY id DESC LIMIT 1")) or die(mysql_error());
				$id=$id[0] + 1;
...
if($Type!="summary"){
    ...
    <table align="left" border="1" class="forumline">
      <tr height="30"> 
        <td class="row3" align="center" width="180" nowrap>Student Name <BR></td>
        <?
...
?>
  <table align="left" border="1" class="forumline">
	...
	/*DISPLAY ASSIGNMENT FOLLOWED BY CATEGORY*/
	  ...
	  $resultCol=execute("SELECT `title`, `max_point`, `inserted_date`, `description`, `apply_grade`, `grade_type` FROM `grade_assessment` WHERE `subject`=$subject and status=1 ORDER BY category_id");
		$rowCount=rowcount($resultCol);
...
$apply_grade";
			...
			?>
                <td class="row3" align="center" title="<?=$titleValue?>" nowrap>&nbsp;
             <font color="#006600"><?=$rCol['title']?><BR><?PHP if($rCol['max_point']==0){ echo 'Letter';}else{ echo $rCol['max_point'];}?></font>&nbsp;</td>
            <?
			}
			else
...
$resultColV=execute("SELECT `category_id`, `title`, `max_point`, `inserted_date`, `description`, `apply_grade`, `grade_type` FROM `grade_assessment` WHERE `subject`=$subject AND status=1 ORDER BY category_id");
