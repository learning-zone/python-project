...
function reload()
{
	document.frm.action='unit_gradekg.php';
	document.frm.submit();
}
...
?>		<table width="50%" align="center" class="forumline" border="1" cellspacing="0" cellpadding="0">
  <tr>
    <td colspan="2" align="center" class="head">ADD SKILL SET</td>
    </tr>
...
?>
  ...
  echo "<tr height='25'>
    ...
    <td nowrap align='center'>&nbsp;$r1[student_id]</td>";
		...
		?>
    <td nowrap align='center'>&nbsp;<a href= "javascript:OpenWind2('unit_gradekg1.php?course=<?=$branch?>&sem=<?=$sem?>&examid=<?=$examname?>&accyeardet=<?=$accyeardet?>&studentid=<?=$r1[id]?>&class_section_id=<?=$class_section_id?>&stundetname=<?=$r1[first_name]?>&student_id=<?=$r1[student_id]?>')">ADD MARKS		
</a>
    </td></tr><?php
