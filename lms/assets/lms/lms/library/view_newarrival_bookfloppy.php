...
<?
	...
	{
	?>
		<div  Class="Label" align="center"><small><b>New Arrival Floppy Details Report </b></small></div>
		<table width="100%">
		<tr>
		<td>
		<b>Media Type:</b>Floppy
		</td>
</tr>
	<?
	}
...
?>
<tr><td><b>Register=</b><?=$reg?></td><td align='right'><b>As on :</b> <?=date('d-m-Y')?></td></tr>
</table>
<?
echo "<center>";
...
if($register!=0)
{
	$qry="select distinct(b.master_id) from  lib_floppy_acc_det b,lib_floppy_det a where ";
	$qry .=" b.register=$register and b.mode='A' and b.floppy_status='1' and b.media_type=$media and b.master_id=a.id and a.date_of_acquiring between '$FromDate' and '$ToDate' order by a.date_of_acquiring ASC";
}else
{
	$qry="select distinct(b.master_id) from  lib_floppy_acc_det b,lib_floppy_det a where ";
	$qry .=" b.mode='A' and b.floppy_status='1' and b.media_type=$media and b.master_id=a.id and a.date_of_acquiring between '$FromDate' and '$ToDate' order by a.date_of_acquiring ASC";
}
	$t4=execute($qry);
	$countRS = rowcount($t4);
	if($countRS==0)
  ...
  $LAST = ((int) $PAGES - 1) * $_NUMREC_;
    ...
    for($i=$SeekPos;$i<$MAX;$i++){
          $row = fetcharray($t4);
		$sql2="select * from lib_floppy_det where id=$row[master_id]";
		$rs22=execute($sql2);
		$r22=fetcharray($rs22);
		$sql="select count(*) from lib_floppy_acc_det where master_id=$row[master_id]";
		$rs=execute($sql);
		$r=fetcharray($rs,0);
			echo "<tr>";
			echo "<tr>";
...
<form name="frm" action="view_newarrival_bookfloppy.php">
