...
$modDet=$_POST['modDet'];
	...
	$sql="select a.*,b.library,b.register,b.media_type from lib_cd_det a,lib_cd_acc_det b where a.id=b.master_id and b.acc_no='$acc_no' and b.cd_status=1 limit 0,1";
	$rs = execute($sql);
	$row= rowcount($rs);
	//if($row==0)
		//{
...
function frm_modify()
	{
		document.modDet.action = "modify_book_cd.php?Type=modify";
...
<BODY>
<form method="POST" name="modDet" action="modify_book_cd_det.php">
<table border="0" class='forumline' colspan='4' align='center' width='60%'>
<input type="hidden" name="acc_no" value="<?=$acc_no?>">
	<tr height='24'>
		<td align='center' class='head' colspan=4>Modify Book CD  Details</td>
	</tr>
	<?php
	$sql="select a.*,b.library,b.register,b.media_type from lib_cd_det a,lib_cd_acc_det b where a.id=b.master_id and b.acc_no='$acc_no' limit 0,1";
	$rs = execute($sql);
	$r=fetcharray($rs,0);
	?>
	<tr>
...
$Register=1;
		...
		<tr>
			<td>Title</td>
			<td colspan=2><input type="text" name="title" value="<?=$r[title]?>" onKeydown="first_caps(this.form.title)"></td>
			<td></td>
		</tr>
		<tr>
			<td>Author</td>
				...
				while($r2=fetcharray($rs1))
					{
						...
						<tr>
							...
							<td>
							<input type="text" name="sel1[]" value="<?=$r3[acc_no]?>"></td>
							<td>
							<input type="text" name="sel2[]" value="<?=$r3[call_no]?>"></td>
							<td>
							<?php
							$sel1="";
							$sel2="";
...
<br>
<div align="center"><input type="button" name="modify" value="Modify" onClick="frm_modify()" class='bgbutton'></div>
</form>
</BODY>
</HTML>