...
<tr>
	<td colspan=5 class=head align=center>View Issued Magazine/Journal/Question Paper Report</td>
</tr>		
<tr>
...
<tr>
	<td width="30%" align="right">From Date&nbsp;&nbsp;&nbsp;</td>
	<?php
	?>
	<td>
	<?php
...
?>
 <br>
    <center>
	<table align=center class=forumline colspan=7 width="90%">
	<tr height='20'>
        <td align='center' colspan=7>As on :<?=date('d-m-Y g:i:s:a')?></td>
    </tr>
	<tr>
...
if($media==0)
{
	...
	$sql.="lib_membership_det b,lib_circulation_m c where b.m_id=a.id and b.mbno=c.cno and c.status=0 and";
	$sql.=" (c.issue_date >= '".$datefrom."') and (c.issue_date <= '".$dateto."') and c.media_type in(7,8) order by c.issue_date";
}
if($media!=0)
{   
	//echo "<p>Media Value:$media</p>";
	$sql="select a.m_no,c.status,c.cno,c.due_date,c.issue_date,c.name,c.acc_id from lib_membership_m a,";
...
?>	
  ...
  <td class="CBody" align='center'><a href="mag_view.php?acc_no=<?=$r["acc_id"]?>&m_no=<?=$r["cno"]?>"><?=$r["acc_id"]?></a></td>
