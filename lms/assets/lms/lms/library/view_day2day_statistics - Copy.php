...
while($rr=fetcharray($rs_sqll))
{
	...
	<tr><td align='center' class=rowpic>Sl No.</td>
	...
	{	
		...
		if($media=='0')
		{
			$rs_sql=execute("select count(a.id) as no_issue from lib_circulation_m a where a.issue_date='$ndate' and library='$rr[0]'");
			//$rs_sql1=execute("select count(a.id) as no_issue from lib_circulation_r a where a.issue_date='$ndate' and library='$rr[0]'");
			$r_sql=fetcharray($rs_sql);
			$r_sql1=fetcharray($rs_sql1);
			$issue=$r_sql["no_issue"]+$r_sql1["no_issue"];
		...
		else
		{
			...
			$rs_sql=execute("select count(a.id) as no_issue from lib_circulation_m a where a.issue_date='$ndate' and library='$rr[0]' and a.media_type='$media'");
			//$rs_sql1=execute("select count(a.id) as no_issue from lib_circulation_r a where a.issue_date='$ndate' and library='$rr[0]' and a.media_type='$media'");
		...
		if($media=='0')
		{
			//$rs_sql=execute("select count(a.id) as no_return from lib_circulation_r a  where a.return_date='$ndate' and library='$rr[0]'");
		}
		else
		{
			//$rs_sql=execute("select count(a.id) as no_return from lib_circulation_r a  where a.return_date='$ndate' and library='$rr[0]' and a.media_type='$media'");
		}
		if(rowcount($rs_sql)>0)
		{
			$r_sql=fetcharray($rs_sql);
			$return=$r_sql["no_return"];
		}
		else
		{
			$return=0;
		...
		if($media=='0')
		{
			$rs_sql=execute("select count(a.id) as no_renewal from lib_circulation_m a where a.ren_date='$ndate' and library='$rr[0]'");
			//$rs_sql1=execute("select count(a.id) as no_renewal from lib_circulation_r a where a.ren_date='$ndate' and library='$rr[0]'");
			$r_sql=fetcharray($rs_sql);
			$r_sql1=fetcharray($rs_sql1);
			$renewal=$r_sql["no_renewal"]+$r_sql1["no_renewal"];
			mysql_free_result($rs_sql);
			mysql_free_result($rs_sql1);
		}
		else
		{
			$rs_sql=execute("select count(a.id) as no_renewal from lib_circulation_m a where a.ren_date='$ndate' and library='$rr[0]' and a.media_type='$media'");
			//$rs_sql1=execute("select count(a.id) as no_renewal from lib_circulation_r a where a.ren_date='$ndate' and library='$rr[0]' and a.media_type='$media'");
			$r_sql=fetcharray($rs_sql);
			$r_sql1=fetcharray($rs_sql1);
			$renewal=$r_sql["no_renewal"]+$r_sql1["no_renewal"];
			mysql_free_result($rs_sql);
		...
		if(($issue !=0) || ($return !=0) || ($renewal !=0) || ($reserved !=0))
		{
			...
			<td align='center'><?php echo $trans_date=date('d-m-Y',strtotime($ndate)); ?></td>
			<? if($issue>0){
			?>
			<td align='center'><a href="javascript:OpenWind('dtodreport.php?library=<?=$rr[0]?>&media=<?=$media?>&ndate=<?=$ndate?>&rtp=1');"><?php echo $issue; ?></a></td>
			<? } 
			else
			...
			<? if($return>0){?>
				<td align='center'><a href="javascript:OpenWind('dtodreport.php?library=<?=$rr[0]?>&media=<?=$media?>&ndate=<?=$ndate?>&rtp=2');"><?php echo $return; ?></a></td>
			<? }
			else
				echo "<td align='center'>$return</td>" ;
			?>
		   <? if($renewal>0){?> 
			<td align='center'><a href="javascript:OpenWind('dtodreport.php?library=<?=$rr[0]?>&media=<?=$media?>&ndate=<?=$ndate?>&rtp=3');"><?php echo $renewal; ?></a></td>
			<?php
			}
			else 
			...
			<? if($reserved>0){?> 
			<td align='center'><a href="javascript:OpenWind('dtodreport.php?library=<?=$rr[0]?>&media=<?=$media?>&ndate=<?=$ndate?>&rtp=4');"><?php echo $reserved; ?></a></td>
			<? }
			else
