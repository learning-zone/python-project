...
<form>
<?
	$sql="insert  into lib_floppy_det (title,class_no,author,rack,date_of_acquiring,key_word1,key_word2,key_word3,key_word4,key_word5,
	...
	for($i=0;$i<$varlen;$i++)
	{
		...
		$sql="insert into lib_floppy_acc_det(master_id,media_type,acc_no,mode,floppy_status,floppy_type,library,register,flag,call_no)";
		$sql.=" values('$master_id','$media','$var2[$i]','A','1','$floppy_type','$library','$register','0','$www')" ;
		execute($sql) or die(mysql_error());
		echo "<br> Accession No <b>$var2[$i] </b>Entered Successfully ";
	
	}
echo "<a href=../library/add_book_floppy.php?media=$media><b>Go Back</b></a></div></td>";
?>
</form>
</BODY>
</HTML>