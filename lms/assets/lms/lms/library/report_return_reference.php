...
<tr>
	<td colspan=5 class=head align=center>View Returned Reference Report</td>
</tr>		
<tr>
...
<tr>
	<td colspan=6 align=center><input type="submit" name="sub1" value="View Returned Reference Media List"  	                class='bgbutton'>
	</td>
</tr>
</table>
</form>
...
if($media==0)
{
$sql="select a.m_no,c.cno,c.due_date,c.issue_date,c.return_date,c.name,c.acc_id,c.ret_to from lib_membership_m a,";
$sql.="lib_membership_det b,lib_reference_media_trans c where c.returned='Yes' and c.status='1' and b.m_id=a.id and b.mbno=c.cno";
$sql.=" and (c.return_date >= '".$datefrom."') and (c.return_date <= '".$dateto."') order by c.issue_date";
}
if($media!=0)
{
$sql="select a.m_no,c.cno,c.due_date,c.issue_date,c.return_date,c.name,c.acc_id,c.ret_to from lib_membership_m a,";
$sql.="lib_membership_det b,lib_reference_media_trans c where c.returned='Yes' and c.status='1' and c.media_type='$media' and";
$sql.=" b.m_id=a.id and b.mbno=c.cno and (c.return_date >= '".$datefrom."') and (c.return_date <= '".$dateto."') order by c.issue_date";
}
$rs = execute($sql);
$row1=rowcount($rs);
if($row1 > 0)
...
<br>
	...
	<tr height='20'>
        <td align='center' colspan=9><b><font size='2'>As on :</b><?=date('d-m-Y g:i:s:a')?></font></td>
    </tr>
	<tr>
		<td colspan=9 align=center class="Head">List of Returned Reference Media</td>
	</tr>
	<tr>
...
?>	
  ...
  <td class="CBody" align='center'><a href="view_acc_info2.php?acc_no=<?=$r["acc_id"]?>&m_no=<?=$r["cno"]?>"><?=$r["acc_id"]?></a></td>
