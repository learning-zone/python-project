...
$quer3=fetchrow(execute("select * FROM `cafeteria_det` where user='$StudID' and user_type=4"));
if($type1==1 || $type2==2)
	{
		$breakfast=1;
	}
...
if($quer3[0])
{
	...
	execute("UPDATE `cafeteria_det` set `user`='$id',`breakfast`='$breakfast',`lunch`='$lunch',`snacks`='$snacks', `dinner`='$dinner',`type1`='$type1',`type2`='$type2',`type3`='$type3',`type4`='$type4',`user_type`='2',`date_entered`='$today',`date_time_entered`='$today_time' where `user_type`='4' and user='$StudID'");
}
else	
{
execute("INSERT INTO `cafeteria_det`(`user`,`breakfast`,`lunch`,`snacks`, `dinner`,`type1`,`type2`,`type3`,`type4`,`user_type`,`date_entered`,`date_time_entered`)VALUES('$StudID','$breakfast','$lunch','$snacks','$dinner','$type1','$type2','$type3','$type4','4','$today','$today_time')");
}
?>
 <script>
...
<?php
$quer3=fetcharray(execute("select * FROM `cafeteria_det` where user='$StudID' and user_type=4"));
 if($quer3['type1']=='1')
  $chec1='checked';
  else
  $chec2='checked';
