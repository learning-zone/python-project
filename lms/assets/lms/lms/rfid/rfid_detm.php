...

{
	...
	$val=fetchrow(execute("select id from rfid_user_access where userid='$StudID' and user_type=4 and status=1"));
...

	{
 			...
 			execute("INSERT INTO `rfid_user_access` (`user_type`, `userid`, `food`, `trans`, `status`) VALUES ( '4', '$StudID', '$accesscaf', '$accesscaf', '1')");
...

{
	...
	$Sql66=execute("select id from rfid_enrolment_user where user='$StudID' and status='1' and user_type=4");
...

	{
		   ...
		   $sql33="INSERT INTO `rfid_enrolment_user` (`rfid`, `user`, `user_type`, `add_date`, `end_date`, `status`) VALUES ('$rfidno1', '$StudID', '4', '$fromd', '$tod', '1')";
...
$dstuds=fetchrow(execute("select m_name,parent_username from student_m where id='$id'"));

$dgrdes=fetchrow(execute("select m_photo from student_photo where studid='$id'"));

$fullnamess=fetchrow(execute("select first_name,last_name from student_m where id='$id'"));

?>

<td align="left" style="font-size:16px" colspan="4"  background="green.png"><b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color="#FFFFFF"><?=$dstuds[0]?></font></b></td>
...
<td align="center" rowspan="7" background="green.png" width="120px"><img src="<?php echo $dgrdes[0]?>"  height='100'></td>

<td align="left" width="70px" style="border-bottom-right-radius:20px" background="green.png" class="ftsize" ><font color="#FFFFFF">&nbsp;&nbsp;&nbsp;<?=$stafffsid[0]?></font></td>
...
<td align="left" colspan="4" height="0px"><b>&nbsp;</b></td>
...
$studact=fetcharray(execute("select rfid, add_date, status from rfid_enrolment_user where user='$StudID' and active='Y' and user_type=4"));
...
$studeat=execute("select rfid, add_date,end_date, status,id from rfid_enrolment_user where user='$StudID' and active='Y' and user_type=4");
