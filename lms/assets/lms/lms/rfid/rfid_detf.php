...

{
	...
	$val=fetchrow(execute("select id from rfid_user_access where userid='$StudID' and user_type=3 and status=1"));
...

	{
 			...
 			execute("INSERT INTO `rfid_user_access` (`user_type`, `userid`, `food`, `trans`, `status`) VALUES ( '3', '$StudID', '$accesscaf', '$accesscaf', '1')");
...

{
	...
	$Sql66=execute("select id from rfid_enrolment_user where user='$StudID' and status='1' and user_type=3");
...

	{
		   ...
		   $sql33="INSERT INTO `rfid_enrolment_user` (`rfid`, `user`, `user_type`, `add_date`, `end_date`, `status`) VALUES ('$rfidno1', '$StudID', '3', '$fromd', '$tod', '1')";
...
<title>Parent  Id Card Personalizer</title>
...
$stafffsid=fetchrow(execute("select f_id from additional_info2 where student_id='$id'"));



$dstuds=fetchrow(execute("select parent_name,parent_username from student_m where id='$id'"));

$dgrdes=fetchrow(execute("select f_photo from student_photo where studid='$id'"));

$fullnamess=fetchrow(execute("select first_name,last_name from student_m where id='$id'"));

?>

<td align="left" style="font-size:16px" colspan="5"  background="green.png"><b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color="#FFFFFF"><?=$dstuds[0]?></font></b></td>
...
<td align="left" width="70" style="border-bottom-right-radius:20px" background="green.png" class="ftsize" ><font color="#FFFFFF">&nbsp;&nbsp;&nbsp;<?=$stafffsid[0]?></font></td>
...
<td align="left" colspan="4" height="30px"  valign="top"><font  style="font-size:13px"><b>&nbsp;
...
<td align="left" colspan="4"  height="30px" valign="top"><font  style="font-size:14px"><b>&nbsp;<b><?=$fullnamess[0]?>&nbsp;<?=$fullnamess[1]?>
...
<td rowspan="4" class="ftsize"  background="buledark.PNG" width="8%"><p class="vertical" style="width:4px"><font color="#FFFFFF"><b><?=$accyeardet?>&nbsp;-&nbsp;<?=$accyeardet+1?></b></font></p></td>
...
<td align="left" width="70" colspan="4" height="0px"><b>&nbsp;</b></td>
...
$studact=fetcharray(execute("select rfid, add_date, status from rfid_enrolment_user where user='$StudID' and active='Y' and user_type=3"));
...
$studeat=execute("select rfid, add_date,end_date, status,id from rfid_enrolment_user where user='$StudID' and active='Y' and user_type=3");
