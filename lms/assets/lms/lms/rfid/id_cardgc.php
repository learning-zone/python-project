...
if($_POST['update'])
{
	...
	$val=fetchrow(execute("select id from rfid_user_access where userid='$StudID' and user_type='$nval' and status=1"));
	if($val[0])
	{
		?>
        <script>
	...
	{
 			execute("INSERT INTO `rfid_user_access` (`user_type`, `userid`, `food`, `trans`, `status`) VALUES ( '$nval', '$StudID', '$accesscaf', '$accesscaf', '1')");
...
if($_POST['save'])
{
	$Sql66=execute("select id from rfid_enrolment_user where user='$StudID' and status='1' and user_type='$nval'");
	if(rowcount($Sql66)>0)
	{
		?>
        <script>
	...
	{
		   $sql33="INSERT INTO `rfid_enrolment_user` (`rfid`, `user`, `user_type`, `add_date`, `end_date`, `status`) VALUES ('$rfidno1', '$StudID', '$nval', '$fromd', '$tod', '1')";
...
if($_POST['delete'])
{
	...
	{
		$k=$sel[$i];
		$rfsdet=execute("update rfid_enrolment_user set `active`='N', status='0' where id='$k'");
	}
			?>
			<Script language="JavaScript">
...
if($_POST['inactive'])
{
	...
	{
		$k=$sel[$i];
		$rfsdet=execute("update rfid_enrolment_user set status='0' where id='$k'");
	}
			?>
			<Script language="JavaScript">
...
if($_POST['active'])
{
	...
	{
		$k=$sel[$i];
		$rfsdet=execute("update rfid_enrolment_user set status='1' where id='$k'");
	}
			?>
			<Script language="JavaScript">
...
<table border="0" width='400' height="245px" align="center"  cellpadding="0" cellspacing="0" class="curvedEdges">
<tr>
...
<tr height="40">
<?php
$dstuds=fetchrow(execute("select s_name,s_photo,new_id from student_photo_other where studid='$id'  and id='$other_id'"));
$fullnamess=fetchrow(execute("select first_name,last_name,student_id from student_m where id='$id'"));


$other_gurds=fetchrow(execute("select relation,s_name,s_photo,id from student_photo_other where  id='$StudID'"));


$stafffsid=fetchrow(execute("select m_id from additional_info2 where student_id='$id'"));

	$name=$fullnamess[2];
	$var=substr($name,1);
	$mids="G".$var;
?>
<td align="left" style="font-size:17px" colspan="3"  background="red.PNG"><b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color="#FFFFFF"><?=$other_gurds[1]?></font></b></td>

</tr>
<tr height="12%">
<td align="center" rowspan="7" width="10"></td>

<td align="center" rowspan="7" background="red.PNG" width="125">
<table border="0" align="center"  cellpadding="0" cellspacing="0" width="100%"  height="100%">
<tr>
<td align="center" valign="top">
<img src="<?php echo $other_gurds[2]?>" height="138">
</td>
</tr>
</table>
</td>
<td align="left" background="red.PNG" class="ftsize" style="border-bottom-right-radius:20px" ><font color="#FFFFFF">&nbsp;<strong><?=$mids?></strong></font></td>
<td align="center" colspan="3"><font color="#D52D12" size="3" style="font-family:'Arial Black', Gadget, sans-serif"><strong>CARE GIVER</strong></font></td>
</tr>

<td align="center" nowrap colspan="4" valign="top">
<table border="0" align="center"  cellpadding="0" cellspacing="0" width="100%"  height="100%">
<tr height="10%">
<td colspan="5"></td>
</tr>
...
<tr>
<td width="3%"><b>&nbsp;</b></td>
<td align="left" colspan="3"  height="30px" valign="top" rowspan="4"><font  style="font-size:15px"><b><?=$other_gurds[1]?>&nbsp;&nbsp;
<?
}
?>
<?php
...
<br>
<?
$studact=fetcharray(execute("select rfid, add_date, status from rfid_enrolment_user where user='$StudID' and active='Y' and user_type='$nval'"));
if($studact[0])
{
	?>
<table align='center' cellpadding="5" class='forumline' width='80%' border="1" >
...
<?php
$studeat=execute("select rfid, add_date,end_date, status,id from rfid_enrolment_user where user='$StudID' and active='Y' and user_type='$nval'");
