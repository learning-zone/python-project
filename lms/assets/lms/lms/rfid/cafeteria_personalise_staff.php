...
$quer3=fetchrow(execute("select * FROM `cafeteria_det` where user='$id1' and user_type=2"));
if($type1==1 || $type2==2)
	{
		$breakfast=1;
	}
...
if($quer3[0])
{
	...
	execute("UPDATE `cafeteria_det` set `user`='$id1',`breakfast`='$breakfast',`lunch`='$lunch',`snacks`='$snacks', `dinner`='$dinner',`type1`='$type1',`type2`='$type2',`type3`='$type3',`type4`='$type4',`user_type`='2',`date_entered`='$today',`date_time_entered`='$today_time' where `user_type`='2' and user='$id1'");
}
else	
{
	
	
execute("INSERT INTO `cafeteria_det`(`user`,`breakfast`,`lunch`,`snacks`, `dinner`,`type1`,`type2`,`type3`,`type4`,`user_type`,`date_entered`,`date_time_entered`)VALUES('$id1','$breakfast','$lunch','$snacks','$dinner','$type1','$type2','$type3','$type4','2','$today','$today_time')");
}
?>
 <script>
...
<tr><td width="35%" align="center">Time</td><td align="center">Type</td></tr>
  ...
  $quer3=fetcharray(execute("select * FROM `cafeteria_det` where user='$id1' and user_type=2"));
