...
if(isset($_POST['update']))
{
	...
	for($i=0;$i<sizeof($cid);$i++)
	{
		...
		//echo "<br>";
		    ...
		    execute("update online_exam_des_mark set score='$scoreval' where ques_id='$cid[$i]' AND student_id='$student_id'");	
...
<?php
$sql31=execute("SELECT student_id FROM `online_exam_des_mark` where exam_id='$idn' and status=1 group by student_id");
while($r1=fetcharray($sql31))
{
	$student_name=mysql_fetch_row(execute("select first_name, last_name from student_m where id='$r1[0]'"));
	if($studentid==$r1[0])
	{
...
<input type="hidden" name="student_id" value="<?php echo $student_id; ?>">
  <?php	
  $sql2=execute("select * from `online_exam_des_questions` where exam_id='$idn' and status='1'");
...
<tr>
	...
	{      
	   ...
	   $sql21=execute("select score from `online_exam_des_mark` where exam_id='$idn' AND student_id='$student_id' and status='1' AND ques_id='$r6[0]' ORDER BY ques_id");
	   $ans=mysql_fetch_row(execute("select Description from `online_exam_des_mark` where exam_id='$idn' AND student_id='$student_id' and status='1' AND ques_id='$r6[0]' ORDER BY ques_id"));
	   $score_disp=fetcharray(execute("select score from `online_exam_des_mark` where exam_id='$idn' AND student_id='$student_id' and status='1' AND ques_id='$r6[0]' ORDER BY ques_id"));
