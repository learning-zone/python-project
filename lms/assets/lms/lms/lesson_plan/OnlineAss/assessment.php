...
function OpenWind2(k2)
{
	...
	{
		document.frm.action="assessment.php";
		document.frm.submit();
	}
	function RefreshMe2(val)
	{
		document.frm.action="assessment.php?id="+val;
		document.frm.submit();	
	}
	function RefreshMe1()
	{
		...
		{
		 	document.frm.action="assessment.php?save=1";
			document.frm.submit();
		}
	}
...
{
	$cid=$_GET['id'];
		execute("update online_assessment set status=0 where id='$cid'");	
		?>
		<Script language="JavaScript">
...
if($_GET['save']==1)
{
	...
	$sql2=execute("SELECT id FROM `online_assessment` where class_id='$sem' and acc_year='$accyear' and section_id='$class_section_id' and exam_name='$ename' and subject_id='$subject' AND exam_type='2' and status=1");
	if(rowcount($sql2)>=1)
	{
		?>
		<Script language="JavaScript">
	...
	else
	{
		...
		execute("INSERT INTO `online_assessment` (`class_id`, `acc_year`, `section_id`, `exam_name`, `exam_short_name`, `subject_id`, `exam_type`, `score`, `exam_date`, `status`, `time_limit`,`timefrt`) VALUES ( '$sem', '$accyear', '$class_section_id', '$ename', '$short_name', '$subject', '2', '0', '$date1', '1', '$ttime','$hurs')");
		?>
		<Script language="JavaScript">
...
<table align='center' class='forumline' width='75%' border="1" >
	...
	<tr>
      	...
      	<td align='center' nowrap>
      	        <textarea name="ename" id="ename" rows="2" cols="60"></textarea>
...
?>
    ...
    <option value="AM" <?=$zero?>>AM</option>
    <option value="PM" <?=$one?>>PM</option>
    </select>
</td>
	</tr>
</table>
...
$sql3=execute(" SELECT * FROM `online_assessment` where class_id='$sem' and acc_year='$accyear' and section_id='$class_section_id' and subject_id='$subject' and status=1");
if(rowcount($sql3)>=1)
{	
	?>
<table align='center' class='forumline' width='75%' border="1" >
