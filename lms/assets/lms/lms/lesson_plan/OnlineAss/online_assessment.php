...
<?php 
	...
	$k=1;
	$sql=execute("select subject_name, subject_id, sub_type, elective from subject_m where course_year_id='$sem' and status=1 ");
	while($r=fetcharray($sql))
	{
		...
		$sql3=execute("SELECT exam_name, id, time_limit, exam_type, exam_date FROM `online_exam_det` where class_id='$sem' and acc_year='$a_year' and section_id='$class_section' and subject_id='$subject' and status=1");
		if(rowcount($sql3)>0)
		{
            ...
            <tr>
				...
				{
				?>
                <a href="add_ans.php?id=<?=$r1[1]?>&examname=<?=$r1[0]?>&time=<?=$r1[2]?>&student_id=<?=$studentid?>"><input type="button" name="update" value="Attend Exam/Result"  class='bgbutton'></a>
                <?php
				}
				else
				{
				?>
                <a href="add_ans1.php?id=<?=$r1[1]?>&examname=<?=$r1[0]?>&time=<?=$r1[2]?>&student_id=<?=$studentid?>"><input type="button" name="update" value="Attend Exam/Result"  class='bgbutton'></a>
                <?php
				}
				?>
                </td>
