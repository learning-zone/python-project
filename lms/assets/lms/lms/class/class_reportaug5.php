...
<select name="test" STYLE="width:150px;height:80px" multiple onClick="reloadme()">
   <?php
    ...
    $rs=execute("select a.id,a.codename,a.section_name from class_section a,subject_m b where b.sub_type=2 and a.sub=b.subject_id and b.course_year_id=a.grade and a.status=1 group by a.id order by  b.course_year_id,a.section_name");
   	...
   	{
   	$rs=execute("select c.id,c.codename,c.section_name from all_teachers a,users b,class_section c,course_year d,subject_m e where b.username='$user'  and e.sub_type=2 and c.id=a.section and c.status=1 and c.sub=e.subject_id and d.year_id=a.class and b.srid IN ( sub_teac2, sub_teac, home_teac) order by a.class, a.section");
   	}
...
?>
  ...
  <?php
   $sectvats = fetchrow(execute("select codename,section_name from class_section where id='$test'")); 
   	$sql123.="select a.id,a.student_id,a.first_name,a.last_name,a.admission_id,a.course_yearsem from student_m a,student_course b where b.stu_id=a.id and b.sub_sec='$test' and b.acc_year='$academic_year' ";
...
<tr>
    ...
    <td  width="10%" class="head"  align="center" nowrap>Course</td>
     <td  width="10%" class="head"  align="center" nowrap>HR Teacher</td>   
    </tr>
  <?php
  $i=1;
  ...
  $allteac= fetchrow(execute("select b.f_name,b.s_name from all_teachers a, staff_det b where a.section='$test' and a.class='$r1[course_yearsem]' and a.home_teac=b.id"));
  echo "<tr>
