...
if($_POST['save'])
{
	...
	if($section!='')
{
	$sql2=execute("select * from class_section where grade='$sem' and sub='$subj' and section_name='$section' and status=1");
	if(rowcount($sql2)>=1)
	{
		?>
		<Script language="JavaScript">
...
if($_POST['dele'])
{
	...
	for($i=0;$i<sizeof($cid);$i++)
	{
        ...
        execute("update class_section set status=0 where id='$cid[$i]'");	
	}
		?>
		<Script language="JavaScript">
...
<table align='center' class='forumline' width='50%' border="1" >
  ...
  <br>
  <div align='center' >
  <input type="submit" name="dele" value="Delete"  class='bgbutton'>
  </div>
	<?php
}
?>	
