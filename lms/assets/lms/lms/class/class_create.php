...
<script>
    $(document).ready(function(){
        ...
        $(".iframe").lightbox({iframe:true, width:"80%", height:"80%"});                                
...
if($sem=="")
{
	$subnamrs=fetcharray(execute("SELECT course_year_id from subject_m where subject_id='$subject' and status='1'"));
if($subnamrs[0]=="0")
{			
			$subject=$subject;
}
...
?>
 <td valign="top" width="20%" nowrap rowspan="2"><b>Course</b><br>
    ...
    <?
		if($sem=="" || $sem=="0")
		{
        	$rs_sub=execute("select * from subject_m  where status=1 order by course_year_id");
		}
		else
		{
        ...
        while($r_sub=fetcharray($rs_sub))
        {
            if($subject==$r_sub[subject_id])
            	echo "<option value='$r_sub[subject_id]' title='$r_sub[subject_code] - $r_sub[subject_name]' selected>$r_sub[subject_code] - $r_sub[subject_name]</option>";
            else
            	echo "<option value='$r_sub[subject_id]' title='$r_sub[subject_code] - $r_sub[subject_name]'>$r_sub[subject_code] - $r_sub[subject_name]</option>";
		}
	...
	$idsnames=0;
	?>
	 <a class='iframe' href="add_sub.php?grades=<?=$sem?>&subj=<?=$subject?>&class_section=<?=$section?>">
	...
	$idsnames=1;
	?>
	<a class='iframe' href="add_sub_new.php?grades=<?=$sem?>&subj=<?=$subject?>&class_section=<?=$section?>">
    ...
    <b>Section</b><br>
    <select name="section"  onClick="reloadme()" multiple STYLE="width:150px;height:150px">
		<?
...
<td valign="top"  nowrap rowspan="3">
   ...
   {
   ?>
   <a class='iframe' href="add_section.php?subj=<?=$subject?>&sem=<?=$sem?>&class_section=<?=$section?>">
   ...
   else
   {
	...
	?>
	  ...
	  {
	  ?>
	  <a class='iframe' href="enroll.php?sem=<?=$sem?>&subj=<?=$subject?>&class_section=<?=$section?>&idsnames=<?=$idsnames?>">
...
&nbsp;
 <a class='iframe' href="student_reports_print.php?sem=<?=$sem?>&subject=<?=$subject?>&class_section_id=<?=$section?>">     
...
<div style="overflow: auto;height:200px; width:550px;" align="center">
    ...
    <?php
	$sql123="select a.id,a.sub,a.sub_sec,b.id,b.student_id,b.first_name,b.last_name from student_course a,student_m b where a.acc_year='$academic_year' and b.archive='N' and a.stu_id=b.id and a.sub='$subject' and a.sub_sec='$section' group by a.stu_id order by b.first_name";	
	$rs=execute($sql123);
  ?>
     <?php
  $i=1;
...
<td valign="top" width="20%" nowrap colspan="2"><b>Grade</b><br>
    ...
    &nbsp;
    <a class='iframe' href="add_grade.php?">   
