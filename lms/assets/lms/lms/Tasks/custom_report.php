...
$objPHPExcel->getProperties()->setCreator("Jack")
	...
	$sql="SELECT pseudo_name, date, file_number, order_number, queue_number, process_status, comments FROM tasks_m WHERE process_status='Completed' AND date='$today'";
...
/*	
			...
			$objPHPExcel->getActiveSheet()->setTitle('Processed Orders');
...
*/
	...
	echo date('H:i:s') , "Continuing..." , EOL;
// 2nd sheet

	$sql="SELECT pseudo_name, date, file_number, order_number, queue_number, process_status, comments FROM tasks_m WHERE process_status='Rejected' AND date='$today'";
	$result=mysql_query($sql);
	...
	if(mysql_num_rows($result)>0)
	{
	   ...
	   $k=1;
		...
		//$objPHPExcel->setActiveSheetIndex(1);
		$objPHPExcel->getActiveSheet(1)->setCellValue('A' . $i, 'NO');
		$objPHPExcel->getActiveSheet(1)->setCellValue('B' . $i, 'Name');
		$objPHPExcel->getActiveSheet(1)->setCellValue('C' . $i, 'Date');
		$objPHPExcel->getActiveSheet(1)->setCellValue('D' . $i, 'File #');
		$objPHPExcel->getActiveSheet(1)->setCellValue('E' . $i, 'Order #');
		$objPHPExcel->getActiveSheet(1)->setCellValue('F' . $i, 'Queue');
		$objPHPExcel->getActiveSheet(1)->setCellValue('G' . $i, 'Status');
		$objPHPExcel->getActiveSheet(1)->setCellValue('H' . $i, 'Comment');
...
/*	
			...
			// Rename worksheet
			echo date('H:i:s') , " Rename worksheet" , EOL;
			$objPHPExcel->getActiveSheet()->setTitle('Rejected Orders');
...
*/
			...
			$user_names="Processed Orders".".php";
