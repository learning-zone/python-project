...
if ($_GET) {
    ...
    $esign_vendor = $_GET['esign_vendor'];
...
if ($_POST) {
    ...
    $esign_vendor = $_POST['esign_vendor'];
...
if($_REQUEST['Type'] == "NEW"){ //FOR NEW RECORD INSERTION
    ...
    $token = $pulling_time = $ending_time = $error_name = $comments = $order_type = ' ';  $page = '-1';
...
<html>
  ...
  <script type="text/javascript">
    function errorFunction(token){
 
        if(token=='E-Signature'){
  ...
  <script type="text/javascript">
    ...
    function Save_onClick() { //alert('INSERT');
     
        document.frm.action = "addtasks_exec.php?Type=INSERT";
        document.frm.submit();
    }

    function Update_onClick() { //alert('UPDATE');
    ...
    function eSignature() {
         ...
         var errors = document.getElementById("error_name").value;
         if(errors!=' '){
              document.getElementById("comments").innerHTML=errors;
         }
    }
    function eSignatureActivate() {
        
        document.frm.action = "addtasks.php";
        document.frm.submit();
    }
</script>
...
<?php
  ...
  // GETTING E-SIGNATURE DETAILS  
      $esdetails=fetcharray(execute("SELECT * FROM `tasks_m_esignature` WHERE `tasks_m_id`='$token' AND status=1 LIMIT 1"));
...
<tr>
    <td valign="top" rowspan="120"  align='left' nowrap class='dropdown' > 
       ...
       ////////////////////////////////////////    
        ...
        while($row=fetcharray($rs))
        {
            ...
            if($row['process_status']=='Completed'){
                ...
                $fontColor = 'red';
            }elseif($row['process_status']=='Start'){
                $fontColor = '#0066FF';           
            }else{
                $fontColor = 'yellow';
            }
  ...
  <?php  // PAGINATION BUTTON
    ...
    <?php
        if(($order_type==2) and strtotime($details['date'])==strtotime($todaydate) || strtotime($details['date'])=='') { //BYDEFAULT HIDE E-SIGNATURE ORDER
            
            ?>
      <tr>
          <td align="center" colspan="4"><input type="button" name="save"  value="Save" class="bgbutton" onClick="errorFunction('E-Signature')" style="width:70px;" /></td>
      </tr>       
            <?
...
<tr>
    ...
    <td colspan="4"><textarea name="comments" id="comments" rows="2" cols="60" required><?=$details[comments]?></textarea></td>
</tr>
<tr> 
    <td align="left" nowrap>&nbsp;Order Type</td>
    <td colspan="4"><select name="order_type" id="order_type" style="min-width: 150px;" onchange="eSignatureActivate();" >
     <option value="">&nbsp;</option>
      <?php
     
         $rso=execute("SELECT `id`, `order_type_name` FROM `tasks_order_type` WHERE status=1");
         
         if(!empty($esdetails[id])){
             $order_type=2;
         }
             
        while($ro=fetcharray($rso))
        {

            if($order_type==$ro['id'])
                echo "<option value='$ro[id]' selected>$ro[order_type_name]</option>";
            else
                echo "<option value='$ro[id]'>$ro[order_type_name]</option>";

        }
    ?></select>
</tr>
<?php
    if($order_type==2) { //IF THE ORDER TYPE IS E-SIGNATURE
        
?>
<tr>
    <td align="left" nowrap>&nbsp;Error Message Orders</td>
    <td colspan="4"><select name="error_name" id="error_name" style="min-width: 150px;" onchange="eSignature();" >
     <option value="">&nbsp;</option>
      <?php

         $rse=execute("SELECT `id`, `error_name` FROM `tasks_error_message` WHERE status=1");
         
         if(!empty($esdetails['error_name'])){
             $error_name=$esdetails['error_name'];
         }
             
        while($re=fetcharray($rse))
        {

            if($error_name==$re['error_name'])
                echo "<option value='$re[error_name]' selected>$re[error_name]</option>";
            else
                echo "<option value='$re[error_name]'>$re[error_name]</option>";
...
<?php
       ...
       $edetails=fetcharray(execute("SELECT * FROM `tasks_m_esignature` WHERE `tasks_m_id`='$token' AND status=1 LIMIT 1"));
         
?>
     <tr>        
         <td nowrap>&nbsp;Borrower’s Name</td>
         <td><input type="text" name="borrower_name" value="<?=$edetails['borrower_name']?>" /></td>
         <td nowrap>Company Name</td>
         <td><input type="text" name="company_name" value="<?=$edetails['company_name']?>" /></td>
         
     </tr>  
     <tr>       
         <td nowrap>&nbsp;E-sign Vendor</td>
         <td colspan="4"><input type="text" name="esign_vendor" value="<?=$edetails['esign_vendor']?>" /></td>
