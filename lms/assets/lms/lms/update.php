...
if($subj!='')
{
	$res_dep = mysql_query("select dept_code from dept_no where dpt_id = '$subj'");
	$row_dep = mysql_fetch_assoc($res_dep);
	
	$res_staff = mysql_query("select max(id) as id from staff_det ");
	$staff_row = mysql_fetch_assoc($res_staff);
	//echo $res_staff; echo "<br>";
	//echo $staff_row; echo "<br>";
	//echo $staff_row[id];
	$res_slno = mysql_query("select max(id) from staff_det ");
	$row_slno = mysql_fetch_row($res_slno);
	$nxt=$row_slno[0];
	echo $nxt = $nxt+1;
	if($nxt<10)
		$nxt="000".$nxt;
...
if(isset($modifyquali)) 
{
	while( list(,$Value) = each($qid) )	
	{
		...
		//$Cspecialization = "specialization" . $Value;
...
if(isset($depmod))
{
	...
	//if($did>0)
	{
		while(list(,$Value)=each($did))
		{
			...
			//echo "update temp_staff_dependents set dname='$dname',drel='$drel',doccu='$doccu',d_addr='$dep_addr12',d_phone='$d_phone' where id=$Value";
...
*/
?>
<tr><td><table width="90%" border="0" cellpadding="0" class="forumline" align="center">
...
<td align="left" valign="top">&nbsp;&nbsp;Rank/Merits/Certificates/Credits Achieved</td>
  ...
  <tr><td>
<table width="90%" border="0" cellpadding="0" class="forumline" align="center">
<tr>	
...
?>  </select>  
  ...
  <td align="left"><input type="text" name="staff_id" size="20" value="<?=$staff_id?>" ></td>
</tr>
<tr align="center" height='30'>
...
<td align="left">Recruited As Per Rules (YES/NO)</td>
	<td align="left">
	<select name="RecPro">
<?php
$tempr="";
...
<br>
<table border=0 class="forumline" width="90%" align="center">
