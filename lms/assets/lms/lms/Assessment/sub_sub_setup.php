...
FROM `exam_sub_sub_m` where exam_id='$examid'  and (exam_name='$ExamName' or exam_sub_name='$ShortName')");
		...
		else
		{
			...
			{
				execute("INSERT INTO `exam_sub_sub_m` ( `exam_name`, `exam_sub_name`, `per_info`, `mark`,  `status`, `order_id`,`exam_id`) VALUES ( '$ExamName', '$ShortName', '$Persatage', '$maxmark','1','$ordercount','$examid')");
				?>
				<Script language="JavaScript">
...
if(isset($_POST['modify']))
{
	...
	for($i=0;$i<sizeof($cid);$i++)
	{
		...
		execute("update exam_sub_sub_m set `exam_name`='$ExamName1', `exam_sub_name`='$ShortName1', `per_info`='$Persatage1', `order_id`='$ordercount1',`mark`='$maxmark1' where id='$cid[$i]'");	
	}
		?>
		<Script language="JavaScript">
		alert("Updated successfully");
...
FROM `exam_sub_sub_m` where exam_id='$examid' ");
