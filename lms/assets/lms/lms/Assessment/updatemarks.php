  ...
  <?php
	...
	if(isset($_POST['save']))
	{
		...
		for($i=0;$i<sizeof($studentid);$i++)
		{
			...
			if($mark1)
			{
				$newquery=execute("select id from $tablename where class_section='$class_section_id' and student_id='$stuid' and subject_id='$subject' and sem_id='$mainid' and int_id='$examsub' and tst_id='$testid'");
				if(rowcount($newquery)>0)
				{
					$sqlquery="update $tablename set mark='$mark1',grade='$grade1', remarks='".addslashes($remarks1)."' where class_section='$class_section_id' and student_id='$stuid' and subject_id='$subject' and sem_id='$mainid' and int_id='$examsub' and tst_id='$testid'";
				}
				else
				{
					if($mark1!='')
					{
						$sqlquery="insert into $tablename(class_section,student_id,subject_id,mark,status, sem_id, int_id, tst_id,grade, remarks) values('$class_section_id','$stuid','$subject','$mark1','1','$mainid' ,'$examsub', '$testid','$grade1','".addslashes($remarks1)."')";
					}
				}
				//echo $sqlquery.'<br>';
				execute($sqlquery);
...
echo "	<input type='hidden' name='course' value='$course'>
	...
	{	
		$sql123.="select a.id,a.student_id,a.first_name,a.last_name from student_m a,student_course b where a.id is not null and a.archive='N'";
		if($branch!=0)
		{
		...
		$sql123.=" and b.stu_id=a.id and b.sub='$subject' and b.acc_year='$accyear'  group by a.id order by first_name ";
	}
 ...
 { 
  ...
  echo "<tr >
	...
	$sql111=execute("select mark, status,remarks,grade from $tablename where class_section='$class_section_id' and student_id='$r1[id]' and subject_id='$subject' and sem_id='$mainid' and int_id='$examsub' and tst_id='$testid'");
