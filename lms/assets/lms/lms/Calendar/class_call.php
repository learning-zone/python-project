...
function go()
{
	document.frm.action="class_call.php";
	document.frm.submit();
}
...
function addnew()
{
		...
		if(tval==1)
		{
			...
			{
				document.frm.action="class_call.php?save=save";
				document.frm.submit();
			}
...
{
	$editid=$_POST['editid'];
	execute("update announcement_class_call set status=0 where id='$editid' ");
	?>
	<SCRIPT LANGUAGE="JavaScript">
...
execute("update announcement_class_call set  fromdate='$fdate', todate='$tdate' ,title='$ename', description='$descr' where id='$editid' ");
	?>
	<SCRIPT LANGUAGE="JavaScript">
...
?>
	...
	<tr >
		...
		$temsql3=execute("select * from announcement_class_call where  id=$stu_id");
...
<br>
	  <div align="center">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  <a href="class_call.php" class='topictitle1'><input type="button" name="Add New" value="Add New" class="bgbutton">
   </a></div>
</FORM>
	<?php
}
...
/*	$temsql=execute("select * from announcement_class_call where title='$ename' and class='$sem' and fromdate='$fdate' and todate='$tdate'");
	...
	else
	{
		*/
		...
		execute("insert into announcement_class_call (class,username,type,fromdate,todate ,title,description,status,grade ,section_id) values('$sem','$user','$type','$fdate','$tdate','$ename','$descr', 1,'$gradeid', '$secid')");
		?>
		<SCRIPT LANGUAGE="JavaScript">
...
<br>
    <table  class='forumline' align='center' width="70%" border="1" cellspacing="0" cellpadding="0">
  <tr>
	<td colspan="4" class="head" align="center">Modify Calendar  </td>
	</tr>
  <tr>
  ...
  <?
	$inc=1;
	$temsql3=execute("select * from announcement_class_call where status=1 and username='$user' and grade!='0'");
	while($r=fetcharray($temsql3))
	{
		echo "
		<tr height='25'>
			...
			<td nowrap>&nbsp;&nbsp;
			<a href='class_call.php?stu_id=$r[id]'>
			$r[title]</a></td>
