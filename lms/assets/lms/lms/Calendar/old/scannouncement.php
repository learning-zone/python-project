...
mysql_query("update announcement set status=0 where id='$stu_id' ");
	?>
	<SCRIPT LANGUAGE="JavaScript">
...
{?>
  ...
  <?
	$inc=1;
	$temsql3=mysql_query("select * from announcement where status=1 and fromdate>='$sytemdate'");
...
mysql_query("update announcement set  fromdate='$fdate', todate='$tdate' ,title='$ename', description='$descr' where id='$stu_id' ");
	?>
	<SCRIPT LANGUAGE="JavaScript">
...
?>
	...
	<?php
		...
		$temsql3=mysql_query("select * from announcement where  id=$stu_id");
	while($r=mysql_fetch_array($temsql3))
	{
		$type=$r['type'];
		$ename=$r['title'];
	...
	?>
	  <tr >
	    <td>&nbsp;&nbsp;Type</td>
	    <td >&nbsp;&nbsp;
	      <select name="type2" id="type2" onChange="go()">
...
if($save=='save')
{
	...
	$temsql=mysql_query("select * from announcement where title='$ename' and class='$FromYear' and fromdate='$fdate' and todate='$tdate'");

	if(mysql_num_rows($temsql)>0)
		echo "<font color='#FF0000'>Duplicate entry not allowed</font> <br>";
	else
	{
		mysql_query("insert into announcement (type,fromdate,todate ,title,description,status) values('$type','$fdate','$tdate','$ename','$descr', 1)");
		?>
		<SCRIPT LANGUAGE="JavaScript">
