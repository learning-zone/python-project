...
if($save=='save')
{
	...
	else
	{
		...
		mysql_query("insert into `announcement_class` (`class`,`username`,`type`,`fromdate`,`todate` ,`title`,`description`,`status`) values('$sem','$user','$type','$fdate','$tdate','$ename','$descr', 1)");
		?>
		<SCRIPT LANGUAGE="JavaScript">
...
if($action=='' and $stu_id=='') 
{
	...
	<br>
	 ...
	 <a href="announcement.php?action=mod" class='topictitle1'><input type="button" name="Modify" disabled value="Modify" class="bgbutton" ></a> </div>
...
<br>
  ...
  <?
	$inc=1;
	//echo "select * from `announcement_class` where `status`=1 and `fromdate`>='$sytemdate' and `class`='$sem'";
	$temsql3=mysql_query("select * from `announcement_class` where `status`=1 and  `class`='$sem'");
	while($r=mysql_fetch_array($temsql3))
	{
		echo "
		<tr height='25'>
