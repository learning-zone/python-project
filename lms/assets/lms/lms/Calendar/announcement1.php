...
if(isset($delete))
{
	execute("update announcement_class set status=0 where id='$stu_id' ");
	?>
	<SCRIPT LANGUAGE="JavaScript">
...
if($action=='mod' and $stu_id=='') 
{
  ...
  <?
	$inc=1;
	$temsql3=execute("select * from announcement_class where status=1 and fromdate>='$sytemdate'");
...
execute("update announcement_class set  fromdate='$fdate', todate='$tdate' ,title='$ename', description='$descr' where id='$stu_id' ");
	?>
	<SCRIPT LANGUAGE="JavaScript">
...
if($save=='save')
{
	...
	{
		execute("insert into announcement_class (class,username,type,fromdate,todate ,title,description,status) values('$branch','$user','$type','$fdate','$tdate','$ename','$descr', 1)");
		?>
		<SCRIPT LANGUAGE="JavaScript">
