...
function go()
{
	document.frm.action="scannouncement.php";
	document.frm.submit();
}
...
function addnew()
{
		...
		if(tval==1)
		{
			...
			{
				document.frm.action="scannouncement.php?save=save";
				document.frm.submit();
			}
...
execute("update announcement_class set status=0 where id='$stu_id' and class='0' ");
	?>
	<SCRIPT LANGUAGE="JavaScript">
...
{?>
  ...
  <tr>
	<td colspan="4" class="head" align="center">Modify Announcement  </td>
	</tr>
  <tr>
  ...
  <?
	$inc=1;
	$temsql3=execute("select * from announcement_class where status=1 and class=0");
	while($r=fetcharray($temsql3))
	{
		...
		<tr >
			...
			<td nowrap>&nbsp;&nbsp;
			<a href='scannouncement.php?stu_id=$r[id]'>
			$r[title]</a></td>
  ...
  $inc++;
    ...
    <br><div class="diva1" align="center"><a href="scannouncement.php" class='topictitle1'><input type="button" name="Add New" value="Add New" class="bgbutton"></a></div><br>
    <?
}
if($update)
{
...
execute("update announcement_class set  fromdate='$fdate', todate='$tdate',username='$user' ,title='$ename', description='$descr' where id='$stu_id' and class=0 ");
	?>
	<SCRIPT LANGUAGE="JavaScript">
...
?>
	...
	<?php
		...
		$temsql3=execute("select * from announcement_class where  id=$stu_id and class=0");
	while($r=fetcharray($temsql3))
	{
		$type=$r['type'];
		$ename=$r['title'];
...
<br>
	 ...
	 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="scannouncement.php" class='topictitle1'><input type="button" name="Add New" value="Add New" class="bgbutton">
   </a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="scannouncement.php?action=mod" class='topictitle1'><input type="button" name="Modify" value="Cancel" class="bgbutton" ></a></div>
</FORM>
	<?php
}
...
if($save=='save')
{
	...
	$temsql=execute("select * from announcement_class where title='$ename' and class='$FromYear' and fromdate='$fdate' and todate='$tdate' and class=0");

	if(rowcount($temsql)>0)
		echo "<font color='#FF0000'>Duplicate entry not allowed</font> <br>";
	else
	{
		execute("insert into announcement_class (type,fromdate,todate ,title,description,status,username) values('$type','$fdate','$tdate','$ename','$descr', 1,'$user')");
		?>
		<SCRIPT LANGUAGE="JavaScript">
...
if($action=='' and $stu_id=='') 
{
	...
	<br>
	  <div align="center">
        ...
        <a href="scannouncement.php?action=mod" class='topictitle1'><input type="button" name="Modify" value="Modify" class="bgbutton" ></a>
	  </div>
