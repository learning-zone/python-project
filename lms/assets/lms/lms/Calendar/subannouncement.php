...
function go()
{
	document.frm.action="subannouncement.php";
	document.frm.submit();
}
...
function addnew()
{
		...
		if(tval==1)
		{
			...
			{
				document.frm.action="subannouncement.php?save=save";
				document.frm.submit();
			}
...
if(isset($delete))
{
	execute("update announcement_subj set status=0 where id='$stu_id' ");
	?>
	<SCRIPT LANGUAGE="JavaScript">
...
execute("update announcement_subj set  fromdate='$fdate', todate='$tdate' ,title='$ename', description='$descr' where id='$stu_id' ");
	?>
	<SCRIPT LANGUAGE="JavaScript">
...
?>
	...
	<?php
		...
		$temsql3=execute("select * from announcement_subj where  id=$stu_id");
	...
	?>	
	  <tr >
	    ...
	    <td >&nbsp;&nbsp;<select name="type2" id="type2" onChange="go()">
	        <option value="1" <?php echo $type1; ?>>
            Single day</option>
	        <option value="2" <?php echo $type2; ?>>
            Multiple day</option>
        </select></td>
      </tr>
	  <tr>
...
<br>
	  <div align="center">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  <a href="subannouncement.php" class='topictitle1'><input type="button" name="Add New" value="Add New" class="bgbutton">
   </a></div>
</FORM>
	<?php
}
...
if($save=='save')
{
	...
	$temsql=execute("select * from announcement_subj where title='$ename' and class='$sem' and fromdate='$fdate' and todate='$tdate'");

	if(rowcount($temsql)>0)
		echo "<font color='#FF0000'>Duplicate entry not allowed</font> <br>";
	else
	{
		execute("insert into announcement_subj (class,username,type,fromdate,todate ,title,description,status) values('$branch','$user','$type','$fdate','$tdate','$ename','$descr', 1)");
		?>
		<SCRIPT LANGUAGE="JavaScript">
...
if($action=='' and $stu_id=='') 
{
	...
	$rs_branch=execute($tempstr) or die(mysql_error());
	while($r1=fetcharray($rs_branch))
	{
	if($branch==$r1[0])
		{
	...
	<br>
	 ...
	 <a href="subannouncement.php?action=mod" class='topictitle1'><input type="button" name="Modify" disabled value="Modify" class="bgbutton" ></a> </div>
...
<br>
  ...
  <?
	$inc=1;
	$temsql3=execute("select * from announcement_subj where status=1 and fromdate>='$sytemdate' and class='$sem'");
	while($r=fetcharray($temsql3))
	{
		echo "
		<tr height='25'>
			...
			<td nowrap>&nbsp;&nbsp;
			<a href='subannouncement.php?stu_id=$r[id]'>
			$r[title]</a></td>
