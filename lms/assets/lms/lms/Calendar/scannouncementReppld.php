							   ...
							   if($das[0]=='Sun')
							   {
										...
										if($todayDate==$bdate)
								   			$newbgcolor='class="headc"';
								   		 	...
								   		 	$newbgcolor='';
												$sql3=execute("SELECT id  FROM `announcement` where ('$bdate' between fromdate and todate and type=2) or (type=1 and fromdate='$bdate')");
												$ro = rowcount($sql3);
												if($ro<=0)
												{
													$avail="No";
								...
								if($avail=="Yes")
								{
								$getcountappointments=execute("SELECT count(id) FROM `announcement` where ('$bdate' between fromdate and todate and type=2) or (type=1 and fromdate='$bdate')");
