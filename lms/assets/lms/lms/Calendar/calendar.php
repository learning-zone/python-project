...
function reload()
{
  document.frm.action='calendar.php';
  document.frm.submit();
}
</script>
<script language="javascript">
...
<table align="center" border="1" cellSpacing="0" style="margin-top:0 px; margin-right:-5 px;" width="100%">
<tr>
							   ...
							   if($das[0]=='Sun')
							   {
										...
										if($todayDate==$bdate)
								   			$newbgcolor='class="headc"';
								   		 	...
								   		 	$newbgcolor='';
												$sql3=execute("SELECT id  FROM `announcement_class` where class=0 and  ('$bdate' between fromdate and todate and type=2) or (type=1 and fromdate='$bdate')");
												$ro = rowcount($sql3);
												if($ro<=0)
												{
													$avail="No";
								...
								if($avail=="Yes")
								{
								$getcountappointments=execute("SELECT count(id) FROM `announcement_class` where class=0 and  ('$bdate' between fromdate and todate and type=2) or (type=1 and fromdate='$bdate')");
