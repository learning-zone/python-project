...
function OpenWind2(k2)
{
	...
	{
		document.MyFrm.action="setup.php";
...
FROM `grade_year_setup` where`acc_year`='$a_year' and `class`='$sem' and (exam_name='$ExamName' or exam_sub_name='$ShortName')");
		...
		{
			if($ExamName!='' and $ShortName!='' and $maxmark!='' )
			{
				execute("INSERT INTO `grade_year_setup` ( `exam_name`, `exam_sub_name`,`mark`, `acc_year`, `status`, `order_id`) VALUES ( '$ExamName', '$ShortName', '$maxmark','$a_year','1','$ordercount')");
					//execute();				
				?>
				<Script language="JavaScript">
...
if($_POST['modify'])
{
	...
	for($i=0;$i<sizeof($cid);$i++)
	{
		...
		execute("update grade_year_setup set `exam_name`='$ExamName1', `exam_sub_name`='$ShortName1', `per_info`='$Persatage1', `order_id`='$ordercount1',`mark`='$maxmark1' where id='$cid[$i]' and status=1 ");	
	}
		?>
		<Script language="JavaScript">
		alert("Updated successfully");
...
<tr>
  <td colspan=2 align='center' class='head'>Declare Year & Term</td></tr>
<tr height="25">
...
<table align='center' class='forumline' width='70%' border="1" >
	...
	<!--	<td align='center' class='head' nowrap> % </td>-->
        <td align='center' class='head' nowrap> Weight </td>
   ...
   <!--   <td align='center' nowrap>
        	<input type='text' name='Persatage' size="3" maxlength="3" value=''>
		</td>-->
...
<?php
		...
		$yearstt=execute("SELECT * FROM `grade_year_setup` where`acc_year`='$a_year'");
...
<table align='center' class='forumline' width='70%' border="1" >
<tr>
<td align="center" colspan="2" class='rowpic'>Grade Book Setup</td>
</tr>
<tr>
<td align="center">
