...
if($_POST['idn'])
{
	...
	for($i=0;$i<sizeof($cid);$i++)
	{
		...
		$optionval=$_POST['optionval'.$cid[$i]];
			...
			$mark=0;
		mysql_query("INSERT INTO `online_exam_sel_mark` (`exam_id`, `ans`, `student_id`, `status`,score) VALUES ('$idn', '$optionval', '$student_id', '1','$mark');");
...
?>
  ...
  <?php
	...
	$total=0;
	$sum=0;
 $sql2=mysql_query("select * from online_exam_sel_questions where exam_id='$idn' and status='1' ORDER BY RAND()");
 ...
 ?>
	...
	{
			$sql21=mysql_query("select score from `online_exam_sel_mark` where exam_id='$r6[0]' AND `student_id`='$student_id' AND status='1'");
			$k=1;
				$score=mysql_fetch_array(mysql_query("select score from `online_exam_sel_mark` where exam_id='$r6[0]' AND `student_id`='$student_id' AND status='1'"));
				$score=$score[0];
