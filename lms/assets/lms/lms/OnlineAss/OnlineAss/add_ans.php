...
if($_POST['idn'])
{
	...
	for($i=0;$i<sizeof($cid);$i++)
	{
		...
		$Questions=$_POST['Questions'.$cid[$i]];
			...
			mysql_query("INSERT INTO `online_exam_des_mark` (`exam_id`, `ques_id`, `Description`, `student_id`, `status`) VALUES ('$idn', '$ques_id', '$Questions', '$student_id', '1');");
...
?>
  ...
  $sql2=mysql_query("select * from online_exam_des_questions where exam_id='$idn' and status='1' ORDER BY RAND()");
...
{	
	...
	{ 	
			...
			$sql21=mysql_query("select Description, score from `online_exam_des_mark` where ques_id='$r6[0]' AND `student_id`='$student_id' AND status='1'");
			$k=1;
				$score=mysql_fetch_array(mysql_query("select score from `online_exam_des_mark` where ques_id='$r6[0]' AND `student_id`='$student_id' AND status='1'"));
				$score=$score[0];
