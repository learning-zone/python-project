...
{
	$eid=$_GET['eid'];
	mysql_query("update online_exam_sel_questions set status=0 where id='$eid'");	
	?>
...
if(isset($_POST['save']))
{
	...
	{
		$sql2=mysql_query("select * from online_exam_sel_questions where exam_id='$idn' and que='$Questions' and status='1'");
		...
		{
			mysql_query("INSERT INTO `online_exam_sel_questions` ( `exam_id`, `no_of_option`, `que`, `option1`, `option2`, `option3`, `option4`, `option5`, `right_ans`, `score`, `status`) VALUES ('$idn', '$countnumber', '$Questions', '$option1', '$option2', '$option3', '$option4', '$option5', '$righans','$score', '1')") or die(mysql_error());
			?>
			<Script language="JavaScript">
...
<br>
  ...
  <br>
  <?php	
  $sql2=mysql_query("select * from online_exam_sel_questions where exam_id='$idn' and status='1'");
