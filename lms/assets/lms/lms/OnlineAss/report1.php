...
<tr>
  ...
  $result=execute("select * from `online_exam_sel_mark` where status='1' GROUP BY student_id");
	 ...
	 $SlNo=1;
      ...
      { 
		  ...
		  $lname=fetcharray(execute("select last_name from student_m where id='$row[0]'"));
	  	  $max_score=fetcharray(execute("select SUM(score) from `online_exam_sel_questions` where exam_id='$idn' and status='1'")); 
	      $obt_score=fetcharray(execute("select SUM(score) from `online_exam_sel_mark` where student_id='$row[0]' AND exam_id='$idn' AND status='1'"));
