<html>
<head><title>Assessment Objective</title>
...
function OpenWind2(k2)
{
	...
	{
		document.MyFrm.action="assessment_cri.php";
...
if ($_POST['update'])
{
	...
	for($i=0;$i<sizeof($cid);$i++)
	{
        ...
        execute("update dpcriteria set criteria='".addslashes($skills)."', posi='$priority' where id='$cid[$i]'");	
	}
		?>
		<Script language="JavaScript">
...
if($skills!='')
{
	...
	$sql2=execute("select * from dpcriteria where acc_year='$accyeardet' and class='$sem' and sub='$subject' and criteria='$skills' and exam_id='$examname_m' and posi='$priority'");
	if(rowcount($sql2)>=1)
	{
		?>
		<Script language="JavaScript">
	...
	else
	{
		...
		$sql5="INSERT INTO dpcriteria (acc_year,class,exam_id,sub, criteria, posi) VALUES ('$accyeardet','$sem','$examname_m','$subject', '".addslashes($skills)."', '$priority')";
		execute($sql5);
		?>
		<Script language="JavaScript">
		alert("Updated successfully");
...
<tr>
  <td colspan=2 align='center' class='head'>Assement Objective</td></tr>
...
?>
 ...
 <tr>
<td>&nbsp;Select Exam</td>
<td><select name='examname_m' onChange="RefreshMe(0)">
  <?
echo "<option value=''>--Select--</option>";
	$sql3=execute("SELECT id,exam_name FROM `dp_exam_sub_m` where `class`='$sem' and status=1 ");
...
<table align='center' class='forumline' width='70%' border="1" >
	<tr>
		...
		<td align='center' class='head' nowrap>Criteria</td>
...
$sql3=execute("select id, criteria, posi from dpcriteria where acc_year='$accyeardet' and class='$sem' and exam_id='$examname_m'");
if(rowcount($sql3)>=1)
{	
	?>
<br>
