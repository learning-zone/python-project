...
if($_POST['save'])
{
	...
	$avg_grade=$_POST['avg_grade'];
			...
			$Sql66=execute("select id from dp_endremark where student_id='$studentid' and exam_id='$examid' and sub='$subject'");
		if(rowcount($Sql66)>0)
		{
			
			
			$sql33="update dp_endremark set `asob`='$asob',`expect`='".addslashes($expect)."',`expct_grade`='$achieve',`punctuality`='$punct',`behavior`='$behav',`attendance`='$attnd',`Class_part`='$cpart',`suggestion`='".addslashes($sug)."',`paper2`='".addslashes($paper2)."',`paper3`='".addslashes($paper3)."',`submission`='".addslashes($sub)."',`inquirer`='$inquirer', `knowledgeable`='$knowledgeable', `open_minded`='$openminded', `caring`='$caring',`thinker`='$thinker', `balanced`='$balanced',`risk_taker`='$risktaker', `principled`='$principled', `communicator`='$communicator', `reflective`='$reflective',`avg_grade`='$avg_grade' where student_id='$studentid' and exam_id='$examid' and sub='$subject'";
			execute($sql33);
		}
		else
		{
			
			execute("INSERT INTO dp_endremark(`class`, `sub`, `exam_id`, `student_id`, `expct_grade`, `suggestion`,`paper2`,`paper3`,`submission`, `punctuality`, `behavior`, `attendance`, `Class_part`, `inquirer`, `knowledgeable`, `open_minded`, `caring`, `thinker`, `balanced`, `risk_taker`, `principled`, `communicator`, `reflective`,`expect`,`avg_grade`,`asob`) VALUES( '$sem','$subject', '$examid', '$studentid', '$achieve','".addslashes($sug)."','".addslashes($paper2)."','".addslashes($paper3)."','".addslashes($sub)."','$punct','$behav','$attnd','$cpart','$inquirer','$knowledgeable','$openminded', '$caring', '$thinker', '$balanced', '$risktaker', '$principled','$communicator', '$reflective','$expect','$avg_grade','$asob')");
		}
...
//$gradenam=fetcharray(execute("select avg_grade,count(avg_grade) from dp_endremark where exam_id='$examid' and avg_grade=5 and sub='$subject' group by avg_grade order by avg_grade"));
  ///////////grade end/////////////
 ?>
...
?>
<tr>
    <td>&nbsp;&nbsp;CLASS TEST GRADE&nbsp;-&nbsp;<b><?=$avg_grade1?></b></td>
    <td colspan="4">&nbsp;&nbsp;EXPECTED GRADE&nbsp;-&nbsp;<b><?=$expect1?></b></td>
</tr>
<tr>
    <td>&nbsp;&nbsp;EXAM GRADE&nbsp;-&nbsp;<b><?=$finlname1?></b></td>
...
<tr>
    ...
    <td>&nbsp;AO&nbsp;<input type="text" align="left" size="10" name="asob" value=<?=$asob?>></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
</tr>
...
<tr>
<td align='center' colspan='10' ><textarea name='paper2' rows='5' cols='80'><?php echo $paper2; ?></textarea></td>
</tr>
<tr>
<td align='center' colspan='10' ><textarea name='paper3' rows='5' cols='80'><?php echo $paper3; ?></textarea></td>
</tr>
<tr>
