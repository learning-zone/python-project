...
function reload()
{
	document.frm.action='perfor_reports_dp.php';
	document.frm.submit();
}
...
?>		<table width="50%" align="center" class="forumline" border="1" cellspacing="0" cellpadding="0">
  <tr>
    <td colspan="2" align="center" class="head">Performance Report for DP FORUM</td>
    </tr>
...
echo "<option value=''>--Select--</option>";
	$sql3=execute("select id, exam_name from dp_exam_sub_m where class='$sem' and status=1 ") or die(mysql_error());
...
?>
  ...
  if($sem=='0' || $sem=='')
	die();
  $sql123.="select id,student_id,first_name,last_name,parent_name  from student_m where id is not null and archive='N' and academic_year='$accyear'";
...
$exmnamst=fetcharray($exmname2);
  ...
  echo "<tr>
	...
	?><a href= "javascript:OpenWind2('perfor_reports_dp1.php?course=<?=$branch?>&sem=<?=$sem?>&subject=<?=$subject?>&examid=<?=$examname?>&studentid=<?=$r1[id]?>&class_section_id=<?=$class_section_id?>&stundetname=<?=$r1[first_name]?>&student_id=<?=$r1[student_id]?>')">Print		
</a>
&nbsp;&nbsp;
<!--<a href= "javascript:OpenWind2('perreportskgword.php?course=<?=$branch?>&sem=<?=$sem?>&subject=<?=$subject?>&examid=<?=$examname?>&studentid=<?=$r1[id]?>&class_section_id=<?=$class_section_id?>&stundetname=<?=$r1[first_name]?>&student_id=<?=$r1[student_id]?>')">Export Word		
</a>--> </td> 
...
<a href= "javascript:OpenWind2('perfor_endreports_dp1.php?course=<?=$branch?>&sem=<?=$sem?>&subject=<?=$subject?>&examid=<?=$examname?>&studentid=<?=$r1[id]?>&class_section_id=<?=$class_section_id?>&stundetname=<?=$r1[first_name]?>&student_id=<?=$r1[student_id]?>')">Print		
</a>
&nbsp;&nbsp;
<!--<a href= "javascript:OpenWind2('perreportskgword.php?course=<?=$branch?>&sem=<?=$sem?>&subject=<?=$subject?>&examid=<?=$examname?>&studentid=<?=$r1[id]?>&class_section_id=<?=$class_section_id?>&stundetname=<?=$r1[first_name]?>&student_id=<?=$r1[student_id]?>')">Export Word		
</a>--> </td>
