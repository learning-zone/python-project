...
if($_POST['save'])
{
	...
	for($j=0;$j<sizeof($manager);$j++)
	{
		...
		$Sql66=mysql_query("select `id` from `staff_leave_notify` where `user`='$user' and `f_date`='$pfdate'  and `t_date`='$ptdate' and `status`=1 ");
...
<tr><td Class="head" align='center' colspan="3">Staff List</td></tr
	...
	?>
	<tr>
	<td align="center"  nowrap><?=$stafff1[1]?></td>
	<td  nowrap>&nbsp;<?=$stafff1[2]?>&nbsp;<?=$stafff1[3]?></td>
    <td align="center"  nowrap><input type="checkbox" name="manager[]" value="<?=$stafff1[0]?>" <?=$check11?>></td>
    </tr>
    <?
