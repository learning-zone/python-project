...
<Script language="JavaScript">
	...
	{
		document.frm.action="leave_jan30.php";
		document.frm.submit();
	}
...
if($_POST['del'])
{
	...
	{
        execute("update staff_leave set 	status_reason='0',status_approve='2',user='$user',status_with_staff='$leave_add' where id='$cid[$g]'");	
	}
		?>
		<Script language="JavaScript">
		alert("Withdraw !");
...
<?
	if($p==1){
		?>
        <li class="currentBtn"><a href="leave_jan30.php?tab=1" >Leave & Attendance</a></li>
        <?
	}else{
		?>
        <li><a href="leave_jan30.php?tab=1" >Leave & Attendance</a></li>
        <?
	}
?>
<?
...
$mang_hrrt=execute("select b.staff_id from users a,staff_hr_grup b where b.status=1 and a.username='$user' and a.srid IN ( b.hr_id,b.mng_id)");
  if(mysql_num_rows($mang_hrrt)>0 || $staffrigtss[0]=='admin')
	...
	if($p==2 || $p==4 || $p==5  || $p==6 || $p==99 || $p==88){
		?>
        <li class="currentBtn"><a href="leave_jan30.php?tab=2" >Leave Approval</a></li>
        <?
	}else{
		?>
        <li><a href="leave_jan30.php?tab=2" >Leave Approval</a></li>
        <?
	}
?>
...
$daycount=fetcharray(execute("select days from staff_leave where staff_id='$staff_id_us[0]' and type='$type'  and status='1' and reject='0' and status_reason!='2'"));
	...
	$daycount_withdrawn=fetcharray(execute("select days from staff_leave where staff_id='$staff_id_us[0]' and type='$type'  and status='1' and reject='1' and status_reason!='2'"));
...
<?
	if($p==4 || $p==2){
		?>
        <li class="currentBtn"><a href="leave_jan30.php?tab=4" >Submitted Leave</a></li>
        <?
	}else{
		?>
        <li><a href="leave_jan30.php?tab=4" >Submitted Leave</a></li>
        <?
	}
?>
<?
	if($p==5){
		?>
        <li class="currentBtn"><a href="leave_jan30.php?tab=5" >Approved Leave</a></li>
        <?
	}else{
		?>
        <li><a href="leave_jan30.php?tab=5">Approved Leave</a></li>
        <?
	}
?>
<?
	if($p==6){
		?>
        <li class="currentBtn"><a href="leave_jan30.php?tab=6">Rejected Leave</a></li>
        <?
	}else{
		?>
        <li><a href="leave_jan30.php?tab=6" >Rejected Leave</a></li>
        <?
	}
?>
<?
	if($p==99){
		?>
        <li class="currentBtn"><a href="leave_jan30.php?tab=99">Withdraw</a></li>
        <?
	}else{
		?>
        <li><a href="leave_jan30.php?tab=99" >Withdraw</a></li>
        <?
	}
?>
<?
	if($p==88){
		?>
        <li class="currentBtn"><a href="leave_jan30.php?tab=88">Default</a></li>
        <?
	}else{
		?>
        <li><a href="leave_jan30.php?tab=88" >Default</a></li>
        <?
	}
?>
</ul>
...
<tr>
  ...
  while($mang_hr_rgts=fetcharray($mang_hr))
	...
	$viewss=execute("select a.type,a.f_date,a.t_date,a.days,a.reason,c.f_name,c.s_name,c.slno,a.id,c.id,a.in_time,a.out_time,a.hd_ee_da_date,a.half_time_in,a.status_reason,a.manager,a.status_approve from staff_leave a,staff_det c where a.status=1  and a.approved='0' and a.reject='0' and c.id='$mang_hr_rgts[0]' and c.id=a.staff_id");
...
$status_text=" ";
 if($viewss1[16]=='2')
	...
	?>
    <tr>
 <td align="center" width="3%"><input type="checkbox" name="manager[]" value="<?=$viewss1[8]?>" <?=$status_check?>></td>
	...
	elseif($viewss1[14]=='2')
	{
		$vatnames11='<font color="#006600">Approved by<br>'."$staff_name_withdrawn[f_name]&nbsp;$staff_name_withdrawn[s_name]".'</font>';
	}
	elseif($viewss1[14]=='3')
	{
		echo '<font color="#FF0000">Rejected by<br>'."$staff_name_withdrawn[f_name]&nbsp;$staff_name_withdrawn[s_name]".'</font>';
	}
	else
	{
		echo "-";
...
<table align='center'  width='90%' border="1" cellpadding="0" cellspacing="0">
  ...
  while($mang_hr_rgts=fetcharray($mang_hr))
	{
	$viewss=execute("select a.type,a.f_date,a.t_date,a.days,a.reason,c.f_name,c.s_name,c.slno,a.id,a.approve_reason,c.id,a.in_time,a.out_time,a.hd_ee_da_date,a.half_time_in,a.status_reason,a.status_approve,a.manager from staff_leave a,staff_det c where a.status=1  and a.approved='1' and c.id='$mang_hr_rgts[0]' and c.id=a.staff_id");
	while($viewss1=fetcharray($viewss))
	{
		$tfdate21=explode('-',$viewss1[1]);
		$fdate21=$tfdate21[2]."-".$tfdate21[1]."-".$tfdate21[0];
...
if($p==1)
	{
$sql3=execute("select id,type,f_date,t_date,backup,days,reason,approved,reject,user_id,approve_reason,reject_reason,hd_ee_da_date,in_time,out_time,half_time_in,status_reason,manager,status_approve,status_approve_manger from staff_leave where status=1  and staff_id='$staff_id_us[0]'  order by id desc");
...
if($r6[1]=='5')
{
 ...
 ?>
  <tr>
  <td align='center'  width="5%"><input type='checkbox' name='cid[]' value='<?=$r6[0]?>' <?=$status_check?>></td>
	...
	elseif($r6[16]=='2')
	{
		...
		echo '<font color="#006600">Leave Withdrawn Approved <br>'."$date_manger".'</font>';
	}
	elseif($r6[16]=='3')
	{
		if(strtotime($r6[19]))
		...
		echo '<font color="#FF0000">Leave Withdrawn Rejected <br>'."$date_manger".'</font>';
	}
...
?>
	...
	elseif($viewss1[15]=='2')
	{
		echo '<font color="#006600">Approved by<br>'."$staff_name_withdrawn[f_name]&nbsp;$staff_name_withdrawn[s_name]".'</font>';
	}
	elseif($viewss1[15]=='3')
	{
		echo '<font color="#FF0000">Rejected by<br>'."$staff_name_withdrawn[f_name]&nbsp;$staff_name_withdrawn[s_name]".'</font>';
...
<tr>
  ...
  while($mang_hr_rgts=fetcharray($mang_hr))
	{
	$viewss=execute("select a.type,a.f_date,a.t_date,a.days,a.reason,c.f_name,c.s_name,c.slno,a.id,a.reject_reason,c.id,a.in_time,a.out_time,a.hd_ee_da_date,a.half_time_in,a.status_reason  from staff_leave a,staff_det c where a.status=1 and  a.status_approve=2  and c.id='$mang_hr_rgts[0]' and c.id=a.staff_id");
	while($viewss1=fetcharray($viewss))
	{
		$tfdate21=explode('-',$viewss1[1]);
		$fdate21=$tfdate21[2]."-".$tfdate21[1]."-".$tfdate21[0];
...
?>
    <tr>
   <td align='center'  width="5%"><input type='checkbox' name='cid_with[]' value='<?=$viewss1[8]?>' ></td>
        <td align="center" width="10%"><?=$viewss1[7]?></td>
        <td width="10%">&nbsp;<?=$viewss1[5]?>&nbsp;<?=$viewss1[6]?></td>
         <td align="center" width="10%"><?=$staflavty[0]?> 
         <?php
        ...
        <td align="justify">&nbsp;<?=$viewss1[4]?></td>
         <td align="justify">&nbsp;<?=$viewss1[9]?></td>
          ...
          if($viewss1[15]=='2')
		  {
			  echo "<font color='#00CC00'>Approved</font>";
		  }
		  elseif($viewss1[15]=='3')
		  {
