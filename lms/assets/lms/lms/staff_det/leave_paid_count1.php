...
/////////////////totals//////
		...
		/*$staff_paid_dis_vatnxt=fetcharray(execute("select * from leave_payroll_total_temp where status=1 and staff_id='$staff_name_dis1[id]'"));
		$staff_paid_dis_vatnxt[paid_vat];
		$staff_paid_dis_vatnxt[un_paid];*/
		...
		/*if($levae_availde_minus<0)
		{
			...
			//	$paid_leaves_tot=$staff_paid_local[0]+$staff_paid_dis_vatnxt[paid_vat];
				// $unpaid_leaves_tot=$staff_paid_local[1]+$staff_paid_dis_vatnxt[un_paid];
				
if($staff_Eligible[0]<=$staff_paid_local[0])
{
	echo "<br>".$staff_paid_local[2].$haiss=0;
}
else
{
	echo "<br>".$haiss=$staff_Eligible[0]-$staff_paid_local[0];
}
