...
if($count=='0')
{
	if(strtotime($today_date)>strtotime($newdateval))
	{
		...
		$satff_absent_date=fetcharray(execute("select id from staff_default where staff_id='$staffnameview[4]' and status=1 and d_date='$newdateval'"));
...
elseif($count=='1')
{
	...
	if(strtotime($today_date)>strtotime($newdateval))
	{
		...
		$satff_default_date=fetcharray(execute("select id from staff_default where staff_id='$staffnameview[4]' and status=1 and d_date='$newdateval'"));
...
elseif($count=='1')
{
	...
	if(strtotime($today_date)>strtotime($newdateval))
	{
		...
		echo "select id from staff_default where staff_id='$staffnameview[4]' and status=1 and d_date='$newdateval'";
		
		$satff_default_date=fetcharray(execute("select id from staff_default where staff_id='$staffnameview[4]' and status=1 and d_date='$newdateval'"));
		//echo "SELECT approved ,reject  FROM `staff_default_status` where staff_id_ins='$satff_default_date[0]'";
		$staff_default_sts=fetcharray(execute("SELECT approved ,reject  FROM `staff_default_status` where staff_id_ins='$satff_default_date[0]'"));		
		if($staff_default_sts[0] == '1')
		{
		echo "<div align='center' style='color:#060'><b>&#9733; P</b></div>";
