...
<?php
			...
			$tdatess=$tfdate[2]."-".$tfdate[1]."-".$tfdate[0];
	...
	if($_POST['subn'])
	{
...

		{
			...
			$sql33="update staff_att_updt set type='$type',user='$user',insert_date='$with_date_test',todtime='$ttimess' where toddate='$tdatess' and staff_id='$staffids'";
...
window.opener.location.href='stafftime_update.php?tab=111&staff_ids='+"<?=$staff_ids?>"+'&adate='+"<?=$adate_old?>"+'&bdate='+"<?=$bdate?>";
		window.close();
...
//////////////////////end///////////////////////	
	...
	if(strtotime($tdatess)<strtotime($from_date_pay) && mysql_num_rows($manager_admin)<1 && $staffrigtss[0]!='admin' && $user!='archanam')
   {
	   ?>
			<script language="javascript">
