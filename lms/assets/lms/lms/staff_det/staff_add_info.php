...
`Ag_St_Date`='$Ag_St_Date',`Ag_En_Date`='$Ag_En_Date',`contract_prd`='$contract_prd',`passport_no`='$passport_no',`country`='$country',`country1`='$country1'`permit_no`='$permit_no',`visa_no`='$visa_no',

`place_of_issue`='$place_of_issue',`renewal_time`='$renewal_time',`owner_name`='$owner_name',`contact_no`='$contact_no',

`owner_address`='$owner_address',`account_no`='$account_no',`bank_name`='$bank_name',`IFCA_code`='$IFCA_code',`uploadedfile`='$uploadedfile',`MBIS_start_date`='$MBIS_start_date',`vendor_name`='$vendor_name',`yrs_of_exp`='$yrs_of_exp' WHERE staff_id='$staff_id'";
...
{ 
	 ...
	 $query="INSERT INTO `staff_additional_info`(`s_staff_id`, `s_name`, `s_add`, `s_mb`, `staff_id`, `staff_category`, `fromdate`, `todate`, `start_contract`,  `end_contract`,`DOI_1`, `DOE_1`, `DOI_2`, `DOE_2`, `DOI_3`, `DOE_3`, `Ag_St_Date`, `Ag_En_Date`, `contract_prd`, `passport_no`, `country`, `permit_no`, `visa_no`, `place_of_issue`, `renewal_time`, `owner_name`, `contact_no`, `owner_address`, `account_no`, `bank_name`, `IFCA_code`,`uploadedfile`,`MBIS_start_date`,`vendor_name`,`yrs_of_exp`,`country1`) VALUES('$s_staff_id', '$s_name', '$s_add', '$s_mb', '$staff_id', '$staff_category', '$fromdate', '$todate', '$start_contract',  '$end_contract','$DOI_1', '$DOE_1', '$DOI_2', '$DOE_2', '$DOI_3', '$DOE_3', '$Ag_St_Date', '$Ag_En_Date', '$contract_prd', '$passport_no', '$country', '$permit_no', '$visa_no', '$place_of_issue', '$renewal_time', ='$owner_name', '$contact_no', '$owner_address','$account_no', '$bank_name','$IFCA_code','$imagepath','$MBIS_start_date','$vendor_name','$yrs_of_exp','$country1')";
...
if($rnum>=1)
{
	...
	while($prow=fetcharray($rrs))
	{
		...
		echo "<select name=contract_num onchange='reload()'>
		...
		echo "<td><input type=text name='contract_num$prow[0]' value='$prow[0]' width='100%'></td>";
		echo "<td><input type=text name='contract_prd$prow[0]' value='$prow[1]' width='100%'></td>";
...
<tr>
	<td><select name="contract_num" onchange='reload()'>
