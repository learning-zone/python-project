...
<Script language="JavaScript">
	...
	{
		document.frm.action="leave_feb19.php";
		document.frm.submit();
	}
...
<?
	if($p==1){
		?>
        <li class="currentBtn"><a href="leave_feb19.php?tab=1" >Leave & Attendance</a></li>
        <?
	}else{
		?>
        <li><a href="leave_feb19.php?tab=1" >Leave & Attendance</a></li>
        <?
	}
?>
<?
...
$mang_hrrt=execute("select b.staff_id from users a,staff_hr_grup b where b.status=1 and a.username='$user' and a.srid IN ( b.hr_id,b.mng_id)");
  if(mysql_num_rows($mang_hrrt)>0 || $staffrigtss[0]=='admin')
	...
	if($p==2 || $p==4 || $p==5  || $p==6 || $p==99 || $p==88){
		?>
        <li class="currentBtn"><a href="leave_feb19.php?tab=2" >Leave Approval</a></li>
        <?
	}else{
		?>
        <li><a href="leave_feb19.php?tab=2" >Leave Approval</a></li>
        <?
	}
?>
...
<?
   ...
   if($type=='0')
   {
	 ...
	 if(strtotime($pfdate) > strtotime($ptdate))
   {
	   ?>
       <Script language="JavaScript">
   ...
   $leave_validate_f_date=execute("SELECT * FROM `staff_leave` WHERE status=1  and (approved='1' or approved='0') and staff_id='$staff_id_us[0]' and status_reason!='2' and ( '$pfdate1' between f_date and t_date and )");
   if(mysql_num_rows($leave_validate_f_date)>=1)
   {
    ...
    $leave_validate_t_date=execute("SELECT * FROM `staff_leave` WHERE status=1 and (approved='1' or approved='0') and staff_id='$staff_id_us[0]' and status_reason!='2' and ( '$ptdate1' between f_date and t_date)");
if(mysql_num_rows($leave_validate_t_date)>=1)
   {
	   if($bdate)
	   {
		...
		?>
        <?php
		$leave_validate_f_date=execute("SELECT * FROM `staff_leave` WHERE status=1  and (approved='1' or approved='0') and staff_id='$staff_id_us[0]' and status_reason!='2'  and hd_ee_da_date='$pfdate1'");
   if(mysql_num_rows($leave_validate_f_date)>=1)
   {
    ...
    $leave_validate_t_date=execute("SELECT * FROM `staff_leave` WHERE status=1 and (approved='1' or approved='0') and staff_id='$staff_id_us[0]' and status_reason!='2' and hd_ee_da_date='$ptdate1'");
if(mysql_num_rows($leave_validate_t_date)>=1)
   {
	   if($bdate)
	   {
...
?>
<td colspan="<?=$colspans_vt?>"  style="font-size:13px" align="center" nowrap>
...
<?
	if($p==4 || $p==2){
		?>
        <li class="currentBtn"><a href="leave_feb19.php?tab=4" >Submitted Leave</a></li>
        <?
	}else{
		?>
        <li><a href="leave_feb19.php?tab=4" >Submitted Leave</a></li>
        <?
	}
?>
<?
	if($p==5){
		?>
        <li class="currentBtn"><a href="leave_feb19.php?tab=5" >Approved Leave</a></li>
        <?
	}else{
		?>
        <li><a href="leave_feb19.php?tab=5">Approved Leave</a></li>
        <?
	}
?>
<?
	if($p==6){
		?>
        <li class="currentBtn"><a href="leave_feb19.php?tab=6">Rejected Leave</a></li>
        <?
	}else{
		?>
        <li><a href="leave_feb19.php?tab=6" >Rejected Leave</a></li>
        <?
	}
?>
<?
	if($p==99){
		?>
        <li class="currentBtn"><a href="leave_feb19.php?tab=99">Withdraw</a></li>
        <?
	}else{
		?>
        <li><a href="leave_feb19.php?tab=99" >Withdraw</a></li>
        <?
	}
?>
<?
	if($p==88){
		?>
        <li class="currentBtn"><a href="leave_feb19.php?tab=88">Default</a></li>
        <?
	}else{
		?>
        <li><a href="leave_feb19.php?tab=88" >Default</a></li>
        <?
	}
?>
</ul>
...
$staff_paid_dis_vat2=fetcharray(execute("select * from leave_staff_paid_tot_acc_temp where status=1 and staff_id='$staff_id_us[0]' and acc_id='6'"));

	$alltotii=$daysvat55[0]-$daycount33[0]-$daycount_withdrawn44[0]-$staff_paid_dis_vat2[paid_vat];
