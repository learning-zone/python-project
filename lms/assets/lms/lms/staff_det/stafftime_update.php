...
<script language="javascript">
	...
	{
		document.frm.action="stafftime_update.php";
		document.frm.submit();
	}
</script>
...
<link type="text/css" rel="stylesheet" href="css/style.css" media="screen"></link>
...
?>
    <td colspan="2" nowrap>&nbsp;&nbsp;Select Staff&nbsp;
        <select name="staff_ids" class="chosen-select" style="width:150px">
        <option value=''>Staff Name</option>     
        <?php
        $typesv=execute("select c.staff_id,a.f_name,a.s_name from staff_det a,users b,staff_hr_grup c where c.status=1 and a.id!='$staffrigtss[1]' and b.username='$user' and b.srid=c.mng_id and a.id=c.staff_id and a.active='YES' group by c.staff_id order by a.f_name");
...
?>
 ...
 <td align="center" nowrap>
   ...
   if($updated_att_staff[1]!='0' && $updated_att_staff[0]=='1')
	...
	<?
	$leavdet1=fetcharray(execute("select * from staff_leave where staff_id='$staff_ids' and ( '$viewdate_att' between f_date and t_date)  and status=1 and status_approve!=2 order by id desc"));

$leave_od_ee=fetcharray(execute("select * from staff_leave where staff_id='$staff_ids' and hd_ee_da_date='$viewdate_att' and status=1 and status_approve!=2 order by id desc"));
