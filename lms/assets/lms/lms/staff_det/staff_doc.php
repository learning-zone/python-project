...
if($_REQUEST['Types'] == "ADD")
{
	...
	for($i=0;$i<$n;++$i)
	{
	  ...
	  $sql="UPDATE `staff_det_doc` SET `description` = '$description' WHERE `id` = '$val'";
...
<script language='javascript'>
	function reloadMe()
	{
		document.frm.action="staff_doc.php";
		document.frm.submit();
	}
	function update()
	{
		document.frm.action="staff_doc.php?Types=ADD";
		document.frm.submit();
	}
</script> 
...
<body>
<form name="frm" method="post" action="staff_doc.php">
<?

if($type=="doc_edt")
{
	
	$result=execute("SELECT * FROM `staff_det_doc` WHERE `status` = 1 AND `staff_id`='$staff_id' ORDER BY id DESC");
