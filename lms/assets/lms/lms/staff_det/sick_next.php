...
<input type="button" class="bgbutton" value="View medical information">
 ...
 <tr class="keyrow"><td width="25%" >&nbsp;Name:</td><td width="25%" ><?=$rtt1[first_name]?>&nbsp;<?=$rtt1[last_name]?><td></td><td></td>
 <tr>
				<td>&nbsp;Select Date</td>
				<td>
...
<td nowrap>&nbsp;&nbsp;Image Upload</td>
          ...
          <input type="radio" name="type"  value="no" onChange="reload2()">Hospital
				...
				<?php
				     if($gob)
				     {
				       ...
				       $c=execute("select * from doc_detail where stud_id='$id' and course_id='$as' and d_date='$ddv'");
  ...
  $check1='checked';			       
				       if(rowcount($c)==0)
				       {
				        ...
				        $gj=execute("insert into doc_detail(stud_id,course_id,age,sex,adm_type,acc_year,doc_name,d_date,time,complaints,treatment,remarks,time_1,type,place,name,uploadedfile )values('$id','$grade','$ag','$gen','$adm','$ad','$doct','$ddv','$timr','".addslashes($txtPresenting)."','".addslashes($txtTreatment)."','".addslashes($txtRecommendations)."','$timr1','$type','$place','$fs','$imagepath')");
						 //echo "SICK REPORT INSERTED SUCESSFULLY";
						
						  $c=execute("select * from doc_detail where stud_id='$id' and d_date='$ddv'");
					   while($c_qry=fetcharray($c))
					   {
						   $doc_detail_id=$c_qry['id'];
						   //$type=$c_qry['type'];
					   ...
					   //echo "INSERT INTO `hospital_det`(doc_name,treatment_date,time_in,time_out,diagnosis,treatment,report,returned,picked,doc_detail_id,hospital_name)values('$doc_name','$treatment_date','$time_in','$time_out','$diagnosis','$treatment','$report','$returned','$picked','$doc_detail_id','$hosp')";
						...
						$query="INSERT INTO `hospital_det`(doc_name,treatment_date,time_in,time_out,diagnosis,treatment,report,returned,picked,doc_detail_id,hospital_name)values('$doc_name','$treatment_date','$time_in','$time_out','$diagnosis','$treatment','$report','$returned','$picked','$doc_detail_id','$hosp')";
