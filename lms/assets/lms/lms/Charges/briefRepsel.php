...
for($i=0;$i<rowcount($rs);$i++)
{
	...
	if(rowcount($chkstud)==0)
	{
		...
		?>
		<td>&nbsp;&nbsp;&nbsp;
		   <A HREF="javascript:OpenWind('billingReport.php?stud_id=<?php echo $r[id]?>&course=<?php echo $r[course_admitted] ?>&sem=<?php echo $r[course_yearsem] ?>&adm_id=<?php echo $r[admission_type] ?>&stud_yr=<?php echo $r[academic_year]?>');"><?php echo $r[student_id] ?></A>		
		  </td>
