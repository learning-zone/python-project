...
<script LANGUAGE="JavaScript">
	function reloadMe()
	{
		document.frm.action="miscfee_cancel_report.php";
		document.frm.submit();
	}
</script>
<script language="javascript">
...
<body>
<form name="frm" method="post" action="miscfee_cancel_report.php">
<br>
...
if($_POST['misceDet'] || $_REQUEST['misceDet'])
{
	   ...
	   $sql="SELECT * FROM `fee_misc_collect_m` WHERE `cancel_status` = 'Y'";
...
if(rowcount($result)>0)
{
   ?>
      ...
      <tr>
		<td class="head" align="center" colspan="8">MISCELLANEOUS CANCEL REPORT</td>
	  </tr>
	  <tr height='22' >
            ...
            <td Class="row3">Cancel Receipt Date</td>	
	   </tr>
       <?php
	      $i=0;
		  $rowclass=1;
	 ...
	 $resultAmount=execute("SELECT a.*,b.* FROM fee_misc_head a, fee_misc_m_desc b WHERE b.id ='$row[m_id]'  AND a.status = 1 AND a.m_id = b.m_id");
		...
		{	 
			...
			?>
                    ...
                    <td align='center' >&nbsp;<? print( date("d-M-Y", strtotime($row[cancelled_date])) ); ?></td>
