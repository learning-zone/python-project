<?php
 ...
 $staffrigtss=fetcharray(execute("SELECT groupname,srid FROM `users` where username='$user'")); 
  
  
  $vatnames=fetcharray(execute("select From_address from mail_settings where user_id='$user' and status=1"));
  if(!$vatnames[0])
  die('<p align=center>Please Enter Email Setting !</p>');
  

	
	if(!$_REQUEST['store_stud'])
	{
		$store_stud=date("his").$staffrigtss[1];
	}
	else
	{
...
{
  document.frm.action="sendmail.php?sendto="+sel;
  document.frm.submit();
}
...
<tr>
    ...
    <select name="person_type" onChange="ReloadMe()" style="width:120px;">
      ...
      <select name="by" style="width:120px;" onChange="ReloadMe()">
         ...
         <option value="status" <?=$four?>>Status</option>
			<?
            if($person_type!="staff" && $person_type!="admission" && $user!='ibdpinfo')
            {
            ?>
         <option value="class" <?=$six?>>Class</option>
         <option value="homeroom" <?=$seven?>>Homeroom</option>  
         <?
			}
		 ?>
      </select>
    ...
    <td align="left" colspan="4" rowspan="2" valign="top">
     <?php
		...
		{
		if($staffrigtss[0]=='adminm' || $staffrigtss[0]=='admin' || $staffrigtss[0]=='ADMIN' || $staffrigtss[0]=='HUMAN RESOURCE')
		{
                    if($by=='status')
                    {
                    ?>
      ...
      <select name="class" multiple style="height:150px; width:160px;" onClick="ReloadMe()">
        <?php
         if($staffrigtss[0]=='adminm' || $staffrigtss[0]=='admin' || $staffrigtss[0]=='ADMIN'  || $staffrigtss[0]=='HUMAN RESOURCE' || $user=='ibdpinfo')
	{
   $sqlClass=execute("select * from class_section a,subject_m b where b.sub_type!=2 and a.sub=b.subject_id and b.course_year_id=a.grade and a.status=1 and a.acc_year='$a_year' group by a.id order by  b.course_year_id,a.section_name");
    while($rC=fetcharray($sqlClass))
          {
   		...
   		{
   	$sqlClass_rs1=execute("select * from all_teachers a,users b,class_section c,course_year d,subject_m e where b.username='$user'  and e.sub_type!=2 and c.id=a.section and c.status=1 and c.acc_year='$a_year' and c.sub=e.subject_id and d.year_id=a.class and b.srid IN ( sub_teac2, sub_teac, home_teac) order by a.class, a.section");
		while($rC=fetcharray($sqlClass_rs1))
          {
			  $tem_arr[] = $rC[id];
          }
	
	 $sqlClass_rs=execute("select * from staff_class_group a,users b,class_section c,course_year d,subject_m e where b.username='$user'  and e.sub_type!=2 and c.acc_year='$a_year' and c.id=a.section and c.status=1 and c.sub=e.subject_id and d.year_id=a.grade and b.srid=a.staff_id order by a.grade, a.section");
		while($rC1=fetcharray($sqlClass_rs))
          {
               $tem_arr[] = $rC1[id]; 
          } 
          $grdnoidss=execute("select * from all_teachers a,users b,class_section c,subject_m e where b.username='$user'  and e.sub_type!=2 and c.id=a.section and c.status=1 and c.acc_year='$a_year' and a.class='0' and c.sub=e.subject_id and b.srid IN ( sub_teac2, sub_teac, home_teac) order by a.class, a.section");
		while($grdnos=fetcharray($grdnoidss))
          {
               $tem_arr[] = $grdnos[id]; 
          } 
		...
		{
		$j=$value;
			$sql1="select id,codename,section_name from class_section where id='".$j."' and status=1 and acc_year='$a_year' order by grade";
			$sqlname=mysql_fetch_row(execute($sql1));
			if($j==$class)
			{
      ...
      <select name="class" multiple style="height:150px; width:160px;" onClick="ReloadMe()">
        <?php
        if($staffrigtss[0]=='adminm' || $staffrigtss[0]=='ADMISSION' || $staffrigtss[0]=='admin'  || $user=='jagrutic'  || $user=='oisinfo' || $user=='eyinfo' || $staffrigtss[0]=='ADMIN'  || $staffrigtss[0]=='HUMAN RESOURCE' || $user=='ibdpinfo')
	{
   $sqlClass=execute("select * from class_section a,subject_m b where b.sub_type=2 and a.sub=b.subject_id and b.course_year_id=a.grade and a.status=1 and a.acc_year='$a_year'  group by a.id order by  b.course_year_id,a.section_name");
    while($rC=fetcharray($sqlClass))
          {
   	...
   	{
   	$sqlClass_rs1=execute("select * from all_teachers a,users b,class_section c,course_year d,subject_m e where b.username='$user' and c.acc_year='$a_year'  and e.sub_type=2 and c.id=a.section and c.status=1 and c.sub=e.subject_id and d.year_id=a.class and b.srid IN ( sub_teac2, sub_teac, home_teac) order by a.class, a.section");
		while($rC=fetcharray($sqlClass_rs1))
          {
			  $tem_arr[] = $rC[id];
          }
	
	 $sqlClass_rs=execute("select * from staff_class_group a,users b,class_section c,course_year d,subject_m e where b.username='$user'  and e.sub_type=2 and c.id=a.section and c.status=1 and c.sub=e.subject_id and c.acc_year='$a_year' and d.year_id=a.grade and b.srid=a.staff_id order by a.grade, a.section");
		while($rC1=fetcharray($sqlClass_rs))
          {
               $tem_arr[] = $rC1[id]; 
          } 
            $grdnoidss=execute("select * from all_teachers a,users b,class_section c,subject_m e where b.username='$user'  and e.sub_type=2 and c.acc_year='$a_year' and c.id=a.section and c.status=1 and a.class='0' and c.sub=e.subject_id and b.srid IN ( sub_teac2, sub_teac, home_teac) order by a.class, a.section");
		while($grdnos=fetcharray($grdnoidss))
          {
               $tem_arr[] = $grdnos[id]; 
          } 
	   ...
	   if($staffrigtss[0]=='adminm' || $staffrigtss[0]=='ADMISSION' || $staffrigtss[0]=='admin'  || $user=='jagrutic'  || $user=='oisinfo'|| $user=='eyinfo' || $staffrigtss[0]=='ADMIN'  || $staffrigtss[0]=='HUMAN RESOURCE' || $user=='ibdpinfo')
			{
				...
				if($user=='ibdpinfo')
				{
					$ibinfo='and year_id>14';
				}
	  ?>
      <select name="class" multiple style="height:150px; width:160px;" onClick="ReloadMe()">
        <?php
   $sqlClass=execute("SELECT `year_id`, `year_name` FROM `course_year` WHERE status=1 $ibinfo ORDER BY `year_id`");
          while($rC=fetcharray($sqlClass))
          {
              if($class==$rC[year_id])
      ...
      ?>
	   ...
	   ?>           
                ...
                <option value='1' <?=$check_cat1?>>Permanent</option>
                <option value='2' <?=$check_cat2?>>Contractual</option>
                </select>
...
<?php
	if($person_type == "staff")
	{
	$sql="SELECT `id`,`f_name` as first_name,`s_name` as last_name FROM `staff_det` where category='$staff_types' and active='YES'  and id!='$adminvats[0]' order by f_name";
	}
	if($person_type == "student")
	{
		if($by!='class' && $by!='homeroom')
		{
			...
			if($staffrigtss[0]!='adminm' || $staffrigtss[0]!='ADMISSION' || $staffrigtss[0]!='admin'  || $user!='jagrutic'  || $user!='oisinfo' || $user!='eyinfo' || $staffrigtss[0]!='ADMIN'  || $staffrigtss[0]!='HUMAN RESOURCE' || $user!='ibdpinfo')
			{
			$sql="select e.id,e.first_name,e.last_name from all_teachers a,users b,class_section c,student_course d,student_m e where b.username='$user' and c.id=a.section and e.id=d.stu_id and c.status=1 and c.grade=a.class and e.archive='N' and e.academic_year='$a_year' and d.sub_sec=a.section and d.sub=a.sub and c.acc_year='$a_year' and b.srid IN ( sub_teac2, sub_teac, home_teac)";
			...
			else
			{
				if($user=='ibdpinfo')
				{
			$sql.=" AND course_yearsem>14";
				}
			}
			echo $sql.=" ORDER BY first_name";
			}
...
if($staffrigtss[0]=='adminm' || $staffrigtss[0]=='ADMISSION' || $staffrigtss[0]=='admin'  || $user=='jagrutic' || $user=='oisinfo' || $user=='eyinfo' || $staffrigtss[0]=='ADMIN'  || $staffrigtss[0]=='HUMAN RESOURCE' || $user=='ibdpinfo')
{
$sql="select a.id,a.student_id,a.first_name,a.last_name,a.admission_id,a.course_yearsem from student_m a,student_course b where b.stu_id=a.id and a.archive='N' and b.sub_sec='$class' and b.acc_year='$a_year'";
...
$sql.=" ORDER BY first_name";
     ...
     <input type="submit" name="addstundet" value=">>" style="width:40px;" class="bgbutton" onClick="form_validate()"/><BR>
     <!--<input type="button" name="selectAll" value=" >> " style="width:60px; height:22px" class="bgbutton"/>-->
     <BR><BR><BR><BR>
     <input type="submit" name="remove" value="<<" style="width:40px;" class="bgbutton" onClick="form_validate()"/><BR>
...
if($_POST['addstundet'])
{
	...
	if($person_type == "staff")
	{
		for($staffid=0;$staffid<sizeof($StudID);$staffid++)
		{
			...
			$staff_id_rs = mysql_fetch_row(execute($staff_sql));
	...
	else
	{
		...
		for($stuid=0;$stuid<sizeof($StudID);$stuid++)
		{
			...
			if($student_type=="pre-Enrolled")
			{
				...
				$students_id = mysql_fetch_row(execute($sql_students_pre));
			...
			{   
				...
				//WHERE id is not null  AND academic_year='$a_year'";
				$students_id1 = mysql_fetch_row(execute($sql_students));
...
<tr>
	...
	{
	?>
    <input type="text"  id="subject" name="subject" size="70" maxlength="70" max="70" disabled placeholder="Please Complete Email Setting!" />
    <?
	}
	?>
    </td>
