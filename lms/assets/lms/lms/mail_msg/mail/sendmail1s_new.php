...
if($check)
{
	...
	{
		$k=$check[$i];
		$senderName1=$senderName[$k];
		$senderEmail12=$senderEmail[$k];
		$senderEmail123=explode(',', $senderEmail12);
		for($m=0;$m<sizeof($senderEmail123);$m++)
		{
			$senderEmail1=$senderEmail123[$m];
			if($senderEmail1!='')
			{
				$parent_mailid= $senderEmail1;
				$sql=rowcount(execute("select id from mail_sent_count where to_mail_id='$parent_mailid' and mail_det_id='$Mail_template'"));				
				
					$parent_name= "Dear  $senderName1 <br>";
