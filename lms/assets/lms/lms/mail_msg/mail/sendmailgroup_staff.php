...
for($i=1;$i<3;$i++)
{
	$sendtnewwal=$_REQUEST['staff_dd'.$i];
	
	if($sendtnewwal==1)
	$sqlst=execute("select id,staff_mailid,first_name,last_name from student_mail_list where staff_mailid!='' and  staff_mailid!='#N/A' and `user`='$user'  and `date_entered`='$sysdate' and store_ids='$store_stud' ");
