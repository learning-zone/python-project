...
if($check)
{
	...
	{
		$sql123=execute("select f_name, email, s_name from staff_det where id='$check[$i]'");
		while($r=fetcharray($sql123))
		{
			if($r[1]!='')
			{
				$parent_mailid1= $r[1];
				$parent_mailid2=explode(',',$parent_mailid1);
				for($k=0;$k<sizeof($parent_mailid2);$k++)
				{
					$parent_mailid=$parent_mailid2[$k];
...
echo "<h5>Out of $t Mail $s Mail Sent Succesfully </h5>\n" ;
?>
</body>
</html>
