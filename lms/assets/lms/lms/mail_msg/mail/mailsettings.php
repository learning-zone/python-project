...
$userdet=fetchrow(execute("select a.email,a.f_name, a.s_name from staff_det a, users b where a.id=b.srid and b.username='$user'"));
$notify_fromname="$userdet[1] $userdet[2]";
$maiid=explode('@',$userdet[0]);

$notify_fromaddress=$userdet[0];
$mail_smtpuser=$maiid[0];
...
if($_POST)
{
	...
	if($notify_fromname!='' and $notify_fromaddress!='' and $Domain_name!='' and $mail_smtpserver!='' and $mail_smtpport!='' and $mail_smtpuser!='' and $mail_smtppass!='')
	{
		$countid=rowcount(execute("select id from `mail_settings` where `user_id`='$user' and status='1'"));
		if(!$countid)
		{
			execute("INSERT INTO `mail_settings` ( `user_id`, `status`, `From_name`, `From_address`, `Domain_name`, `SMTP`, `SMTP_Port`, `Username`, `Password`, `Count`,`Signatore`) VALUES ( '$user', '1', '$notify_fromname', '$notify_fromaddress', '$Domain_name', '$mail_smtpserver', '$mail_smtpport', '$mail_smtpuser', '$mail_smtppass', '0','$Signatore')");
		}
		else
		{
			execute("update `mail_settings` set From_name='$notify_fromname', `From_address`='$notify_fromaddress', `Domain_name`='$Domain_name', `SMTP`='$mail_smtpserver', `SMTP_Port`='$mail_smtpport', `Username`='$mail_smtpuser', `Password`='$mail_smtppass', `Signatore`='$Signatore' where `user_id`='$user' and status=1");
		}
	?>
...
<tr>
            ...
            <tr>
              ...
              <td colspan="3"><input id="notify_fromaddress" name="notify_fromaddress" tabindex="1"  onBlur="validateForm(this.value)"  size="40" maxlengtd="128" value="<?php echo $notify_fromaddress; ?>" type="text"></td>
            </tr>
              <tr id="smtp_autd2">
                      <td scope="row" widtd="20%" nowrap>Domain Name</td>
                      <td colspan="3"><input id="Domain_name" name="Domain_name" size="25" onBlur="" maxlengtd="64" value="thoughtfocus.com" tabindex="1" type="text" readonly></td>
                    </tr>
            <!--<tr>
 ...
 <input type="button" name="other"  class="bgbutton" value="other" onClick="gmail1(3)">
                    ...
                    <tr id="mailsettings1">
                      ...
                      <input id="mail_smtpserver" name="mail_smtpserver" tabindex="1" size="25" maxlengtd="64" value="smtp.gmail.com" type="text" readonly></td>
                      <td width="103" scope="row" widtd="20%">SMTP Port</td>
                      <td width="115" widtd="30%"><input id="mail_smtpport" name="mail_smtpport" tabindex="1" size="5" maxlengtd="5" value="465" type="text" readonly></td>
                    </tr>
                    <tr id="smtp_autd1">
                      <td scope="row" widtd="20%" nowrap>Username</td>
                      <td colspan="3"><input id="mail_smtpuser" name="mail_smtpuser" size="40" maxlengtd="64" value="<?php echo $mail_smtpuser; ?>" tabindex="1" type="text"></td>
                    </tr>
                    <tr id="smtp_autd2">
                    ...
                    <tr >
                      <td scope="row" widtd="20%" nowrap>Signature</td>
