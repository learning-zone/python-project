...
?>
	  ...
	  $phonenum=fetchrow(execute("SELECT msgphone , rgmailid , first_name FROM student_m where id='$studentid'"));
	$stuname=fetchrow(execute("SELECT first_name FROM student_m where id='$studentid'"));
...
for($i=0;$i<=$arrsize;$i++)
{
	...
	$markqury=execute("select $assmk,b.subject_code,$ba from $tablename a, subject_m b where a.studid='$studentid' and a.subid='$subid[$i]' and b.subject_id=a.subid");
