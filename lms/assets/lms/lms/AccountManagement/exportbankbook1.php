...
$yr=date('Y');
		...
		echo "<tr >
                <td colspan=7><div align=right><strong>To Balance Brought Down=&gt;";
				 $qry21=execute("select max(opdate) from ac_opbal where vledger=\"$ledger\" and opdate<\"$dt1\" and vins=\"$ins\" order by iIdx_op");
				 $ab=fetchrow($qry21);
	...
	$s=0;$td=0;$tw=0;
	$sq1=execute("select * from ac_voucher where ddate between \"$bd1\" and \"$bd2\" and acc=\"$ledger\" and iIdx_organization=\"$or1\" order by(ddate)");
	while($row1=mysql_fetch_assoc($sq1))
	{
	$ids=$row1[vvoucherno];
	$ivch=$row1[iIdx_vouchermaster];
	$sq2=execute("select * from ac_voucher where ddate between \"$bd1\" and \"$bd2\" and acc<>\"$ledger\" and iIdx_vouchermaster=\"$ivch\" and vvoucherno=\"$ids\" and iIdx_organization=\"$or1\" order by(ddate)");
	...
	if($row2[Dr_Cr]=="Cr"){$s=$s+$row2[fcredit]+$b;}if($row2[Dr_Cr]=="Dr"){$s=$s-$row2[fdebit]+$b;}
	  ...
	  if($row2[Dr_Cr]=="Dr"){
               ...
               echo "<td>$row2[fdebit]</td> ";} else { echo " <td></td>"; } 
                echo "<td >";if($row2[Dr_Cr]=="Cr"){echo number_format($s,2);$td=$td+$row2[fcredit];}if($row2[Dr_Cr]=="Dr"){echo number_format($s,2);$tw=$tw+$row2[fdebit];}
				echo "</td> </tr>";
                ...
                <td colspan=8><label>
                  <div align=right>
                    <p><strong>Total Deposit: 
                      ...
                      echo "<strong>Total Withdrawal:          </strong>";
                      echo number_format($tw,2)."<br>";
                      echo "<strong>Balance:";
                        $tb=$b+$td-$tw;
						if($tb>0) {
						echo number_format($tb,2)."Dr"."<br>";
						}else {
						 if($tb<0){ 
						 echo number_format(($tb*-1),2)."Cr"."<br>"; 
						 }else{ 
						 echo number_format($tb,2)."<br>";}}
                     echo " </strong> </p>
            <p>&nbsp;</p>
          </div>
          </label></td>
