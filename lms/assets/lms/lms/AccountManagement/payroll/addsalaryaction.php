...
//acount intigration code strats
	...
	$ttlamt=$nsalary;
	$sysdate=$date;
	$cashdetBal=mysql_query("select fopbal from ac_ledger where iIdx_ledger=15");
	$traildetBal=mysql_query("select fopbal from ac_ledger where iIdx_ledger=27");
	$salaryBal1=mysql_fetch_row($cashdetBal);
	$hdfcbankBal1=mysql_fetch_row($traildetBal);

	$salaryBal11=$salaryBal1[0]+$ttlamt;
	$hdfcbankBal11=$hdfcbankBal1[0]-$ttlamt;
	$newRemarks=strtoupper($amt_str);

	mysql_query("update ac_ledger set fopbal='$salaryBal11' where iIdx_ledger=15");
	mysql_query("update ac_ledger set fopbal='$hdfcbankBal11' where iIdx_ledger=27");

	mysql_query("INSERT INTO ac_opbal (opdate, Vledger, fopbal, iId_grp, vins, Dr_Cr, iIdx_organization) VALUES
	('$sysdate', 'Salaries', $salaryBal11, 19, 'Bangalore School', 'Dr', 1),
	('$sysdate', 'Bank HDFC', $hdfcbankBal11, 20, 'Bangalore School', 'Dr', 1)");

	mysql_query("INSERT INTO ac_voucher (iIdx_ledger, iIdx_vouchermaster, iIdx_institution, ddate, Dr_Cr, particulars, chequedd_no, chequedd_date, fdebit, fcredit, vvoucherno, vnarration, acc, iIdx_group, istatus, iIdx_organization, vbillno, dbilldate) VALUES
	(15, 1, 1, '$date', 'Dr', 'By Salaries', '$checknumber', '$checkdate', '$nsalary', '0.00', '$vno1', '$narr', 'Salaries', 19, 0, 1, '$or1', '$billdate'),
	(27, 1, 1, '$date', 'Cr', 'To Bank HDFC', '$checknumber', '$checkdate', '0.00', '$nsalary', '$vno1', '$narr', 'Bank HDFC', 20, 0, 1, '$or1', '$billdate');
	");
...
//acount intigration code ends
	...
	echo "<script>alert('Data added successfully');window.location.href='salarypayment1.php';</script>";
}
else
{
	echo "<script>alert('Already salary issued ');window.location.href='salarypayment1.php';</script>";
}
?>
