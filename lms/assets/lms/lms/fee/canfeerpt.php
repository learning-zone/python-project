...
b.amount , b.clearedDate, b.amountCleared, b.receipt ,b.modeOfPament,b.installmentId from student_m a , fee_m_collect b,fee_apply_fee_student c where  a.id=b.studentId and b.accYear='$a_year' and b.installmentId='$intl' and ((currencyType=1 and modeOfPament!=1) or (currencyType!=1)) 
and a.id=c.student_id and  c.acc_year='$a_year' and  c.status='1' and  b.status='0'";
	if($receiptno)
	{
...
<tr><td align='center' class='head' colspan='9'>Cancelled Fee Report  
</td>
</tr>
<tr height='30'>
...
<td Class="rowpic" align='center' nowrap>Installment </td>
...
for($i=0;$i<rowcount($rs);$i++)
{
	...
	$currencydes1=fetchrow(execute("select code from fee_m_currency_code where id=1"));
		...
		?><td align="center"><A HREF="javascript:OpenWind('feerep.php?recordid=<?php echo $r[id]?>&course=<?php echo $r[course_admitted] ?>&sem=<?php echo $r[course_yearsem] ?>');"><?php echo $r[student_id] ?></A>		
		  </td>
		...
		echo "<td align='center'>$r[installmentId]</td>";
