...
<table class='forumline' align='center' width='90%'><tr><td Class="Head" colspan='5' align='center'>Student wise Fee Report</td></tr>
  <tr>
...
b.amount , b.clearedDate, b.amountCleared, b.receipt ,b.modeOfPament from student_m a , fee_m_collect b,fee_apply_fee_student c where a.id=b.studentId and b.accYear='$a_year' and b.installmentId='$intl' and ((currencyType=1 and modeOfPament!=1) or (currencyType!=1)) 
...
<tr><td align='center' class='head' colspan='8'>Student wise Fee Report  
</td>
</tr>
<tr height='30'>
...
for($i=0;$i<rowcount($rs);$i++)
{
	...
	$currencydes1=fetchrow(execute("select code from fee_m_currency_code where id=1"));
		...
		?><td align="center"><A HREF="javascript:OpenWind('feerep.php?recordid=<?php echo $r[id]?>&course=<?=$r[course_admitted]?>&sem=<?php echo $r[division] ?>&recordid=<?=$r[id]?>');"><?php echo $r[student_id] ?></A>		
		  </td>
