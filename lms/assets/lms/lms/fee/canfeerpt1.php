...
if($dtdiff>=0)
{
	$sql=execute("select distinct(a.pid),a.sid,b.course_abbr,c.year_name from fee_payment a,course_m b,course_year c where a.ins_dt between '$fmyr1' and '$toyr1' and a.recptstatus=1 and a.pid=b.course_id and a.sid=c.year_id order by a.pid,a.sid");

	if(rowcount($sql)>0)
	{
		?>
		<table class='forumline' border=1 align=center>
		<tr height="30"><td align=center class=head colspan=4><font size='3'><u>CANCELLED FEE REPORT</u></font><br>From : <?=$fmyr?> - To : <?=$toyr?></TD></TR>
		...
		{
			$sql1=fetcharray(execute("select sum(pdamt) from fee_payment where pid='$r[0]' and sid='$r[1]' and ins_dt between '$fmyr1' and '$toyr1' and recptstatus=1"));
			...
			echo "<td align='right'><a href=javascript:OpenWind('canfeerpt2.php?pid=$r[0]&sid=$r[1]&fmyr1=$fmyr1&toyr1=$toyr1')>$amt</a></td></tr>";
			$sno++;
			$ttlamt+=$sql1[0];
		}
