...
$pid=$_REQUEST['pid'];
$sid=$_REQUEST['sid'];
$fmyr1=$_REQUEST['fmyr1'];
$toyr1=$_REQUEST['toyr1'];
...
<tr height="30"><td align=center class=head colspan=4><font size='3'>FEE COLLECTION REPORT</font></TD></TR>
...
<?php
$sql=execute("select distinct(a.studid),b.student_id,b.first_name,b.last_name from fee_payment a,student_m b where a.pid='$pid' and a.sid='$sid' and a.ins_dt between '$fmyr1' and '$toyr1' and a.recptstatus=0 and a.studid=b.id order by b.first_name");
$sno=1;
$ttlamt=0;
while($r=fetcharray($sql))
{
	$sql1=fetcharray(execute("select sum(pdamt) from fee_payment where pid='$pid' and sid='$sid' and ins_dt between '$fmyr1' and '$toyr1' and recptstatus=0 and studid='$r[0]'"));
	...
	echo "<td>&nbsp;&nbsp;$r[1]</td>";
	echo "<td>&nbsp;&nbsp;$r[2] $r[3]</td>";
	echo "<td align='right'><a href='feecol3.php?studid=$r[0]&pid=$pid&sid=$sid&fmyr1=$fmyr1&toyr1=$toyr1'>$amt</a></td></tr>";
	$sno++;
	$ttlamt+=$sql1[0];
}
$ttlamt=number_format($ttlamt,2)
?>
<tr><td align='right' colspan='3'>Total Amount&nbsp;&nbsp;</td><td align='right'><?=$ttlamt?></td></tr>
</table><br>
