...
function frm_reload()
{
	document.frm.action='SearchReceipt.php';
	document.frm.submit();
} 
function OpenWind(k)
{
...
<table class='forumline' align='center' width='90%'><tr><td Class="Head" colspan='5' align='center'>Cancel Fee Receipt </td></tr>
  <tr>
...
for($i=0;$i<rowcount($rs);$i++)
{
	...
	$currencydes1=fetchrow(execute("select code from fee_m_currency_code where id=1"));
		...
		?><td align="center"><A HREF="javascript:OpenWind('cancel.php?recordid=<?php echo $r[id]?>&course=<?php echo $r[course_admitted] ?>&sem=<?php echo $r[division] ?>');"><?php echo $r[student_id] ?></A>		
		  </td>
