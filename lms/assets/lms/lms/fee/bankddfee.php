...
function send()
{
	...
	document.frm.action='bankddfee1.php';
	document.frm.submit();
}
function frm_reload()
{
	document.frm.action='bankddfee.php';
	document.frm.submit();
} 
</script>
<?php
...
if($num > 0)
{
	...
	<center>
    <table class='forumline' align='center' ><tr><td Class="Head" colspan='2' align='center'>Bank Demand Draft Report</td></tr>
    </center>
	<tr height='30'>
	<td>Program </td>
		<td>
		...
		<tr height='30'><td>Section </td><td><select name="sect">
			<option value='0'>No Section</option>
			<?php
				$rs=execute("SELECT * from class_section");
				while($r=fetcharray($rs))
				{
					if($sect==$r[0])
					{
