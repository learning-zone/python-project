...
?>
            	...
            	<td colspan="2">
                    ...
                    <tr height="100">
                        <td align="right" colspan="2">
							...
							$yer=$a_year+1;
							?>
                        <th align="center" colspan="4" nowrap="nowrap" bgcolor="#CCCCCC"><font size="+2"><i>Fee Receipt for Academic  Session <?=$a_year."-".$yer?></i></font></th>
  ...
  <div style="background-image:url(http://www.ecole.myschoolone.com/renew/images/EcoleWatermark.png)">
        <table align="center" border="0" cellpadding="5" cellspacing="0" width="90%" height="200">
          <tr height="30">
...
{        
	...
	{
		echo "<tr height='4'><td colspan=4><hr><table align='center' border='0' cellpadding='5' cellspacing='0' width='100%'>";
		...
		<td width='right'  nowrap>DD/Cheque Date : </td>		
	...
	if(!$fine)
	$fine=0;
	echo "<tr  height='4'><td colspan=4><br><hr></td></tr>";		
...
-->
            ...
            <tr>
              ...
              <td align='' colspan="3" ><?=$currencydes[1]?>&nbsp;<?=ucwords(number_to_words($amt))?> only&nbsp;&nbsp;<?=$currencydes[1]?>
            	</td>
            </tr>          
          <tr>
          ...
          <tr><td></td>  
			...
			Fee receipt is subjected to the realization of cheque(s)
            <br><br><br><br>
            </td>
          </tr>
          <tr>
...
if(!rowcount($validation))
{
	...
	for($i=0;$i<sizeof($feeRecVal);$i++)
	{
		...
		{
			$sqlinst="INSERT INTO `fee_m_head_inst_collected` (`uid`, `accYear`, `instId`, `receipt`, `studentId`, `feeHead`, `totalAmount`, `totalConverted`, `currency`, `status`,`totalAmountDet`, `totalDisAmount`, `discountType`) VALUES ('$uid', '$a_year', '$installmentId', '$docid', '$stud_id', '$feeType1', '$feeRecValfinal', '$feeRecValfinal', '$currency', '1','$feeRecVal1', '$feeRecValdis1','$studentdiscountid[0]')";
		}
		else
		{
			$sqlinst="INSERT INTO `fee_m_head_inst_collected` (`uid`, `accYear`, `instId`, `receipt`, `studentId`, `feeHead`, `totalAmount`, `totalConverted`, `currency`, `status`,`totalAmountDet`, `totalDisAmount`, `discountType`) VALUES ('$uid', '$a_year', '$installmentId', '$docid', '$stud_id', '$feeType1', '$feeRecValfinal', '', '$currency', '1','$feeRecVal1', '$feeRecValdis1','$studentdiscountid[0]')";
