...
if($generate)	
{
	...
	for($i=0;$i<sizeof($newst);$i++)
	{
		...
		$fee1=execute("select id from  fee_slab_student where student_id='$id' and acc_year='$a_year' ");
		if(rowcount($fee1)>0)
		{
			$sql1="update `fee_slab_student` set `status`='$sts', slab_id='$nextyear' where student_id='$id'  and acc_year='$a_year' ";
		}
		else
		{
			$sql1="INSERT INTO `fee_slab_student` (`student_id`,  `acc_year`, `status`,`slab_id`) VALUES ('$id', '$a_year', '$sts','$nextyear')";
		}
		//echo $sql1.'<br>';
		execute($sql1);		
...
<table class='forumline' align='center' width="90%" ><tr>
  <td Class="Head" colspan='7' align='center'>Apply  Slab to Student</td></tr>
<tr height='30'>
...
<td Class="rowpic" align='center'>Slab
...
for($i=0;$i<rowcount($rs);$i++)
{
    ...
    <td align="center">
		...
		$fee1=fetchrow(execute("select slab_id from  fee_slab_student where student_id='$r[id]' and acc_year='$a_year' and status='1' "));
		
		
        ?>
        <select name="slab<?=$r[id]?>">
<option value='0'>---Select---</option>
<?php
$rs9=execute("SELECT id, name FROM `fee_discount_slab` where status=1");
while($r9=fetcharray($rs9))
{
	if($fee1[0]==$r9[id])
	{
		echo "<option value='$r9[id]' selected>$r9[name]</option>";
	}
	else
	{
		echo "<option value='$r9[id]'>$r9[name]</option>";
	}
}
?>
</select>
