...
if($_GET[Type]=="ADD"){
	...
	$m_det=mysql_fetch_array(mysql_query("SELECT master_id FROM `lib_acc_details` WHERE library='$library' AND register='$register' AND acc_no='$accNo' AND master_id!=0 ORDER BY id LIMIT 1"));
	
	$acc_det=mysql_fetch_array(mysql_query("SELECT acc_no FROM `lib_acc_details` WHERE library='$library' AND register='$register' ORDER BY id DESC LIMIT 1"));
	...
	//echo "<BR>".$sqlInsert;
	$registerIns=mysql_query($sqlInsert) or die(mysql_error());
	...
	<td><input type="text" name="subtitle" value="<?php echo $r[subtitle]?>" size="60"></td>
      ...
      <tr>  
        <td>&nbsp;&nbsp;&nbsp;&nbsp;Title</td>
			<td><select name='subject' onChange="reloadAjax(this.value)">
             <option value="">---  Select Title ---</option>
				<?php
                $sqlt=mysql_query("SELECT * FROM `lib_book_title` WHERE status=1 ORDER BY id");
       ...
       <tr>
          ...
          <?php
            $sqls=mysql_query("SELECT * FROM `lib_book_subtitle` WHERE status=1 AND `lib_book_title_id`='$subject' ORDER BY id");
