...
</table>-->ADMISSION FORM</td></tr>
<tr><td align='center'>
<table border='0' align='center' width='100%' class='forumline'>
<tr height="25">
	<td colspan='4' class="submenu" ><b>Admission details </b></td>
</tr>
<tr height="25">
...
<?
	...
	$sqlF="SELECT `id`, `display_name` FROM `student_m_field` WHERE status=1 AND `tab_id`=1 ORDER BY `order`";
...
<tr><td>
 ...
 <table border='0' align='center' width='100%' class='forumline'>
	...
	?>
	<td>&nbsp;&nbsp;Age</td>
    <td><input type="text" name="age_yr" value="<?php echo $age_yr?>" size='2' readonly></td>
	<td nowrap>&nbsp;&nbsp;Gender* </td>
 ...
 <?
	...
	$sqlF="SELECT `id`, `display_name` FROM `student_m_field` WHERE status=1 AND `tab_id`=2 ORDER BY `order`";
	$resultF=execute($sqlF) or die(mysql_error());
	  $i=1;
	  while($rf=fetcharray($resultF))
	  {
...
<tr>
 ...
 <?
	...
	$sqlF="SELECT `id`, `display_name` FROM `student_m_field` WHERE status=1 AND `tab_id`=3 ORDER BY `order`";
	$resultF=execute($sqlF) or die(mysql_error());
	  $i=1;
	  while($rf=fetcharray($resultF))
	  {
...
<td align='center'><input type="text" name="goadd" value="<?php echo $goadd ?>"></td>			
 ...
 <?
	...
	$sqlF="SELECT `id`, `display_name` FROM `student_m_field` WHERE status=1 AND `tab_id`=4 ORDER BY `order`";
	$resultF=execute($sqlF) or die(mysql_error());
	  $i=1;
	  while($rf=fetcharray($resultF))
	  {
...
<textarea rows="3" cols="25" name='per_addr' value=""><?=$per_addr?></textarea>
   ...
   <?
	...
	$sqlF="SELECT `id`, `display_name` FROM `student_m_field` WHERE status=1 AND `tab_id`=5 ORDER BY `order`";
	$resultF=execute($sqlF) or die(mysql_error());
	  $i=1;
	  while($rf=fetcharray($resultF))
	  {
...
<tr height="25">
   ...
   <td class="submenu" ><b>Documents Enclosed(Tick the relevent documents)</b></td>
...
<tr height="25">
	...
	<tr>
		...
		<td><input name='password' type="text" value="<?=$app_num?>" size='15' readonly></td>
	</tr>
	<tr>
		<td>&nbsp;&nbsp;Parent Username</td>
		<td><input name='parent_username' type='text' value="<?=$papp_num?>" size='15' readonly></td>
		<td>&nbsp;&nbsp;Parent Password</td>
		<td><input name='parent_password' type="text" value="<?=$papp_num?>" size='15' readonly></td>
	</tr>
    
  <tr>
    <?
	   
	$sqlF="SELECT `id`, `display_name` FROM `student_m_field` WHERE status=1 AND `tab_id`=7 ORDER BY `order`";
	$resultF=execute($sqlF) or die(mysql_error());
	  $i=1;
	  while($rf=fetcharray($resultF))
	  {
...
<?php
	  ...
	  $sqlT="SELECT `id`, `name` FROM `student_m_tab` WHERE status='1' AND id > 7 ORDER BY `id`";
		...
		{			
			?>
				 <td colspan="4" class="submenu"><b>&nbsp;<?=$rt['name']?></b></td></tr>
	        <?
			$sqlF="SELECT `id`, `display_name`, `field_name` FROM `student_m_field` WHERE tab_id = '$rt[id]' ORDER BY `id`";
			...
			while($rf=fetcharray($resultF))
		    {
				?>
                	 <td>&nbsp;&nbsp;<?=$rf['display_name']?></td>
                     <td><input type="text" name="<?=$rf['field_name']?>" value="" size="20"></td>
                <?
...
?>
	...
	<!--<td colspan="4" class="submenu"><b>&nbsp;&nbsp;Hello</b></td>
</tr>-->

</table><br>
<p align='center'><input type="submit" value="Save Details" name="save" class='bgbutton'></p>
