...
<td colspan='6' align='left' class='head' nowrap><b>ANNEXURE-II<?php echo $cd ?></b></td></tr>
<tr><td class='head' align='center' colspan='<?=$hcno?>'>Name of the College :VIDYAVARDHAKA COLLEGE OF ENGINEERING, MYSORE 
</td>
<td  class='head' align='left' colspan='6' nowrap>Region & College Code : 1 DS </td></td></tr>
<tr><td align='center' colspan='<?=$hcno?>' class='head'><?=$wyu[0]?> (UG COURSE)</td>
...
while($ss=fetcharray($adg))
{
	...
	<?
	$df=fetcharray(execute("select intake from sanction_intake ")); 
	...
	{
		$wert=fetcharray(execute("select count(id) from student_m where gender='M' and cetcategory='$ww[id]' and course_admitted='$ss[0]' and  archive='N'"));

		$wer=fetcharray(execute("select count(id) from student_m where gender='F' and cetcategory='$ww[id]' and course_admitted='$ss[0]' and  archive='N'"));
	...
	$wertt_o=fetcharray(execute("select count(id) from student_m where gender='M' and (cetcategory='0' || cetcategory='34') and course_admitted='$ss[0]' and  archive='N'"));
	
	$wert_o=fetcharray(execute("select count(id) from student_m where gender='F' and (cetcategory='0' || cetcategory='34') and course_admitted='$ss[0]' and  archive='N'"));
...
while($ww=fetcharray($sdgk))
{
$wert=fetcharray(execute("select count(id) from student_m where gender='M' and cetcategory='$ww[id]' and  archive='N'"));

$wer=fetcharray(execute("select count(id) from student_m where gender='F' and cetcategory='$ww[id]'  and archive='N'"));
...
$wert_ot=fetcharray(execute("select count(*) from student_m where gender='M' and (cetcategory='0' || cetcategory='34') and  archive='N'"));

$wer_ot=fetcharray(execute("select count(*) from student_m where gender='F' and (cetcategory='0' || cetcategory='34') and  archive='N'"));
