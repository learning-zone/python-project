...
function ReloadMe()
{
    document.frm.action="consolidated_report.php";
	document.frm.submit();
}
</script>
...

{
	...
	<form method='post' action="consolidated_report.php" name="frm" >
...
<!--<td Class="rowpic" align='center'>School Division</td>-->
<td Class="rowpic" align='center'>Grade</td>

<!--<td Class="rowpic" align='center'>Admission No</td>-->

<td Class="rowpic" align='center' width="25%">Student Name</td></tr>
...
<?php
  ...
  $sno=1;
...

	{
		...
		<td align="center">&nbsp;&nbsp;
        <a href="consolidated_report_view.php?StudID=<?php echo $r[id]?>&app_num=<?php echo $app_num ?>&class_section_id=<?php echo $class_section_id ?>&branch=<?php echo $branch ?>&sem=<?php echo $sem ?>&studfname=<?php echo $studfname ?>&a_year=<?php echo $a_year ?>&un=<?php echo $un ?>"><?=$r[student_id]?></a></td>
   
        <?php 
				$course_sql=mysql_fetch_array(mysql_query("select course_yearsem,course_admitted from student_m where id='$r[id]'"));				
				
$year=mysql_fetch_array(mysql_query("select year_name from course_year where year_id='$course_sql[0]'"));
$course=mysql_fetch_array(mysql_query("select coursename from course_m where course_id='$course_sql[1]'"));
?>
<!--<td align="center">
<?=$course[coursename]?>
...
<?=$year[year_name]?>
		...
		<td align="center">&nbsp;&nbsp;<?=$r[first_name]?>&nbsp;<?=$r[last_name]?></td></tr>
