...
$bransql="select year_id, year_name from course_year where ";
  ...
  <?php
	...
	$sql1=execute($bransql);

	while($row1=fetcharray($sql1))

	{

		$sql3=execute("select count(id) from student_m where gender='M' and course_yearsem='$row1[year_id]' and academic_year='$academic_year' and archive='N'");

		$male=fetchrow($sql3);

		$sql4=execute("select count(id) from student_m where gender='F'  and course_yearsem='$row1[year_id]' and academic_year='$academic_year'  and archive='N'");
