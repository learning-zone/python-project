...
<script>
    function cli()
  {
     document.frmMedicaldetail.action='daily_edit_staffs.php';
...
<form name="frmMedicaldetail" method="post" action='daily_edit_staff_next.php'>
...

{
	...
	window.open(finalVar,'Staff','width=900,height=550,status=yes,toolbar=no,scrollbars=yes,menubar=no,location=no');
			...
			<tr>
			<td vAlign="top" align="Center" height="30"  colspan=4 class=head>Edit Daily Treatment Details for -
             <?php 
			$s_name=mysql_fetch_row(mysql_query("select f_name,s_name from staff_det where id='$staff_id'"));
			echo $s_name[0]." ".$s_name[1];
			?></td>
...
<input type="hidden" name="tempiddet" value="<?=$staff_id?>">
...
<input type="button" class="bgbutton" value="View medical information">
		         ...
		         <?php
                              ...
                              $rfg=execute("select * from doc_staff where id='$id'");
							  echo "select * from doc_staff where id='$id'";
			      $fgb=fetcharray($rfg);
...
?>
         ...
         <tr vAlign="top" align="left">
								...
								$qry123=mysql_query("select hospital_name from hospital_tab where id=$qry[hospital_name]");
								while($qw=mysql_fetch_array($qry123))
								{
        ...
        <input type="radio"  name="returned" value="yes" <?=$check?>>School
          ...
          <input type="radio"  name="returned" value="no" <?=$check1?>>Home
				...
				<?php
				     ...
				     $upd=execute("update doc_staff set doc_name='$doct',d_date='$penal',time='$timr',complaints='$txtPresenting',treatment='$txtTreatment',remarks='$txtRecommendations',additional_info1='$additional_info1',additional_info2='$additional_info2',additional_info3='$additional_info3',emergency_name='$emergency_name',emergency_no='$emergency_no',time_1='$timr1',type='$type12' where id='$fgb[id]'")
					  ;
