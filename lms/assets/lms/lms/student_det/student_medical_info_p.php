...
if($gob)
{
	mysql_query("update student_health set checked=0 where student_id = '$student_id' ");
	if(is_array($check))
	{
		while( list(,$value)=each($check))
		{
			...
			echo $selsql = "SELECT * from student_health where student_id = '$student_id' and dname = '$ce' ";
			...
			{
				$var12="update student_health set checked=1 where student_id = '$student_id' and dname = '$ce'";
			}
			echo $var12;
			mysql_query($var12);
		}
...
for($i=0;$i<rowcount($sql);$i++)
{
	...
	$r1=fetcharray($sql,$i);
	$qry=mysql_fetch_row(mysql_query("select checked from student_health where student_id='$student_id' and dname=$r1[id]"));
...
<table border='0' align='center' width='90%' class='forumline' cellspacing="5" >
...
<table width="90%" border="1" align="center" cellspacing="5">
