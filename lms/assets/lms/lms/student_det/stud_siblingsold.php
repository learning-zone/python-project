...
$styd=1;
$fmcode2=execute("SELECT family_code FROM `stud_sibling` where `stud`='$stuid' and `status`=1");
$fmcode76=fetcharray(execute("SELECT family_code FROM `stud_sibling` where `stud`='$stuid' and `status`=1"));
...
{	
	if($lnupdte>2)
	{
		...
		{
			$coacid=$studids[$j];
			$Sql66=execute("select id from stud_sibling where family_code='$ecoleids' and stud='$coacid'");
			if(rowcount($Sql66)>0)
			{
			...
			{
			execute("insert into stud_sibling (`family_name`, `family_code`, `relation`, `stud`, `status`) values('$family_names', '$family_code' ,'$relation', '$coacid','1')");
			}
		}
		
		//this for first insert
	//$studsgg=execute("select id from stud_sibling where family_code='$ecoleids' and stud='$stuid'");
//			if(rowcount($studsgg)<=0)
//			{
//	execute("insert into stud_sibling (`family_name`, `family_code`, `relation`, `stud`, `status`) values('$family_names', '$family_code' ,'$relation', '$stuid','1')");
//			}
			$studsts=sizeof($studids);			
			if($studsts)
			{	
				$newstud=execute("select id from stud_sibling where family_code='$ecoleids' and stud='$stuid'");
				if(rowcount($newstud)>0)
				{
...
$fmnamess=fetcharray(execute("SELECT family_name,relation FROM `stud_sibling` where stud='$stuid' and `status`=1"));
?>
...
<?
$stys=1;
$fmcod=execute("SELECT family_code FROM `stud_sibling` where `stud`='$stuid' and `status`=1");
$fmcodes=fetcharray(execute("SELECT family_code FROM `stud_sibling` where `stud`='$stuid' and `status`=1"));
if(rowcount($fmcod)>0)
{
	$stys=0;
?>
...
<?
/*$fmcode=fetchrow(execute("SELECT family_code FROM `stud_sibling` where `stud`='$stuid' and `status`=1"));	
	$stcodess=execute("SELECT a.stud,b.id,b.first_name,b.last_name,b.student_id,b.admission_id FROM stud_sibling a,student_m b where a.family_code='$fmcode[0]' and a.status=1 and a.stud=b.id");
	while($stcds=fetcharray($stcodess))
	{
		if($stcds[1])
		$check11='checked';
		else
...
$gs='A';
$fmcode=fetchrow(execute("SELECT family_code FROM `stud_sibling` where `stud`='$stuid' and `status`=1"));	
	$stcodess=execute("SELECT a.stud,b.id,b.first_name,b.last_name,b.student_id,b.admission_id FROM stud_sibling a,student_m b where a.family_code='$fmcode[0]' and a.status=1 and a.stud=b.id and a.stud!='$stuid'");
	while($stcds=fetcharray($stcodess))
	{
			if($stcds[0])
			$check11='checked';
...
$fmcodevat=fetchrow(execute("SELECT family_code FROM `stud_sibling` where `stud`='$stuid' and `status`=1"));
	
	$stcode=fetchrow(execute("SELECT stud FROM `stud_sibling` where `family_code`='$fmcodevat[0]' and `status`=1 and stud='$r[id]'"));
