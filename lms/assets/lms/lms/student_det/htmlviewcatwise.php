...
<tr><td class='head' align=center colspan=11>COURSE WISE NUMBER OF STUDENTS ADMITTED All TO SEMESTER</td></tr>
...
while($ss=fetcharray($adg))
{
	...
	$scm=fetcharray(execute("select count(id) from student_m where academic_year='$ac[0]' and course_admitted='$ss[course_id]'  and (quota_id=26 or quota_id=27 or quota_id=28 or quota_id=29) and gender='M'"));
	$stm=fetcharray(execute("select count(id) from student_m where academic_year='$ac[0]' and course_admitted='$ss[course_id]' and (quota_id=31 or quota_id=32 or quota_id=33 ) and gender='M'"));
	$scf=fetcharray(execute("select count(id) from student_m where academic_year='$ac[0]' and course_admitted='$ss[course_id]'  and (quota_id=26 or quota_id=27 or quota_id=28 or quota_id=29) and gender='F'"));
	$stf=fetcharray(execute("select count(id) from student_m where academic_year='$ac[0]' and course_admitted='$ss[course_id]'  and (quota_id=31 or quota_id=32 or quota_id=33 ) and gender='F'"));
	$tof=fetcharray(execute("select count(id) from student_m where academic_year='$ac[0]' and course_admitted='$ss[course_id]' and gender='M'"));
	$tom=fetcharray(execute("select count(id) from student_m where academic_year='$ac[0]' and course_admitted='$ss[course_id]'  and gender='F'"));
	$sno++;
