...

{
	...
	<form method='post' action="select_stud_mod2.php" name="frm" >
...
die();
	...
	$sql="select id,student_id,usn,first_name,last_name,admission_id from student_m where id is not null  and course_admitted is not null and archive='N' and academic_year='$academic_year'";
	//$sql="select id,student_id,usn,first_name,last_name,admission_id from student_m where id is not null and archive='Y' and academic_year = '$academic_year'";
...
<tr><td align='center' class='head' colspan='7'>Modify Student Details</td>

</tr>

<tr height='25' >

<td Class="row3" align='center'>Sl No</td>

<td Class="row3" align='center'>Student ID</td>
<!--<td Class="row3" align='center'>School Division</td>-->
<td Class="row3" align='center'>Grade</td>

<!--<td Class="row3" align='center'>Admission No</td>-->

<td Class="row3" align='center' width="25%">Student Name</td></tr>
...
<?php
  ...
  $sno=1;
...

	{
		...
		<td align="center">&nbsp;&nbsp;
        <a href="modify_Apl.php?StudID=<?php echo $r[id]?>&app_num=<?php echo $app_num ?>&class_section_id=<?php echo $class_section_id ?>&branch=<?php echo $branch ?>&sem=<?php echo $sem ?>&studfname=<?php echo $studfname ?>&a_year=<?php echo $a_year ?>&un=<?php echo $un ?>"><?=$r[student_id]?></a></td>
   
        <?php 
				$course_sql=fetcharray(execute("select course_yearsem,course_admitted from student_m where id='$r[id]'"));				
				
$year=fetcharray(execute("select year_name from course_year where year_id='$course_sql[0]'"));
$course=fetcharray(execute("select coursename from course_m where course_id='$course_sql[1]'"));
?>
<!--<td align="center">
<?=$course[coursename]?>
...
<?=$year[year_name]?>
		...
		<td align="left">&nbsp;&nbsp;<?=$r[first_name]?>&nbsp;<?=$r[last_name]?></td></tr>
