...
if($tmid!="" && $medtyp!='')
{
	...
	if($fgk!=99)
	{
		...
		if($memtp==1)
		{
			$q2=mysql_query("SELECT issues FROM cir_parameter WHERE member='$memtp' AND course='$ccc1' AND media='$media'");
			if(mysql_num_rows($q2)==0)
				$q2=mysql_query("SELECT issues FROM cir_parameter WHERE member='$memtp' AND course='0' AND media='$media'");
			$qq=mysql_fetch_array($q2);
		}
		else
		{
			$q2=mysql_query("SELECT issues FROM cir_parameter WHERE member='$memtp' AND department='$ccc1' AND media='$media'");
			if(mysql_num_rows($q2)==0)
				$q2=mysql_query("SELECT issues FROM cir_parameter WHERE member='$memtp' AND department='0' AND media='$media'");
			$qq=mysql_fetch_array($q2);
		}
	...
	?>
		...
		<?php
		$msg2 = mysql_num_rows($ww);	
		for($i=0;$i<$msg2;$i++)
		{
			$msg3=mysql_fetch_array($ww);
        	...
        	<td>
            	<table border='0' align='center' class='forumline' width='90%'>
					...
					<tr height="30">
                        ...
                        <td colspan="4"><input type='text' name='barcode' value='<?php echo $barcode?>' size=70></td>
                    </tr>
					<tr>
