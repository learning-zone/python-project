...
function cli()
{
	document.form1.action="viewadd_main.php";
...
<tr>
  <td align=center colspan=4  class=head>View Student Medical  Report</td></tr>
	<tr height='30'>
...
<tr>
  <td align='center' class='head' colspan='5'>Student Details</td>
</tr>
<tr height='25' >
...
<?php
  ...
  $sno=1;
	for($i=0;$i<rowcount($rs);$i++)
	{
		...
		echo "select * from stud_health where courseid='$sem' and studid='$r[student_id]'";
		 $qq=execute("select * from stud_health where courseid='$sem' and studid='$r[student_id]'");
		 while($ss=fetcharray($qq))
		 {
				...
				<td>&nbsp;&nbsp;
				<a href='view_medical.php?grade=<?php echo $ss[courseid]?>&branch=<?php echo $branch?>&class_section_id=<?php echo $class_section_id?>&sem=<?php echo $ss[courseid]?>&ad=<?php echo $ss[academicyear]?>&st=<?php 
						 echo $fname?>&sid=<?php echo $ss['studid']?>'>
						 <?php echo $ss[studid]?> </a>  </td>
