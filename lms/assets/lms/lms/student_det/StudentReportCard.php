...
<div class="tabContainer">
 ...
 <table class="forumline"  align="center" width="100%">
  <tr><td valign="top"><BR>   
    ...
    <li class="currentBtn"><a href="StudentReportCard.php?StudID=<?=$StudID?>" title="Student Report">Assessment</a></li>
...
if($StudID!='')
{
	...
	$dirName="Portfolio/".$details['admission_id'];
	$dir = dir("$dirName");
	?>
    <table class=forumline width="100%" cellspacing="1" cellpadding="1" align='center' border="1">
    <tr height="30">
    	<td align='center' Class='row3' colspan='2'>REPORT-CARD &nbsp;&nbsp;[&nbsp;<?=$details['first_name']?>&nbsp;<?=$details['last_name']?>&nbsp;]</td>
    </tr>
    <tr>
    <?
	
	//List files in folder directory
	$sno=1;
	$rowclass=1;
	...
	while (($file = $dir->read()) !== false)
	{
		 ...
		 $url="../Portfolio/$details[admission_id]/$file";
		 $urlDisplay="$file";
		 /*<iframe src="../Portfolio/<?=$admission_id?>/<?=$file?>" scrolling="yes" height="550" width="100%"><?=$file?></a><br/>	*/
		 if($sno > 4) 
		 {	
		
		?>
          <td align='center'>Report <?=($sno-4)?></td>
          <td align='center' title="Click to View Report"><a href="javascript:void(0);" onClick ="OpenWind2('<?=$url?>', 'OpenWind2',1000,800)"><?=$urlDisplay?></a></td>
          </tr>          
        <?
		 }
		$sno++;
