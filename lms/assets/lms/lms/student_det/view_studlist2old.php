...
<tr><td align='center' class='head' colspan='4'><b>Student List</b></td>
<?php
if($clid !='a')
...
{
	if($secid !='a')
		$sql1=execute("select distinct(a.course_yearsem),b.year_name from student_m a,course_year b where a.course_yearsem=b.year_id and a.quota_id='$secid' order by a.course_yearsem ");
	else
...
{
	if($secid !='a')
		$sql2=execute("select id , name from category where id='$secid'");
	else
		$sql2=execute("select distinct(a.quota_id) , b.name from student_m a, category b where a.course_yearsem='$r1[0]' and a.quota_id=b.id order by a.quota_id");

	while($r2=fetcharray($sql2))
	{
		echo "<tr height='30'><td class='rowpic' colspan='4'>&nbsp;&nbsp;&nbsp;Class : $r1[1]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Category : $r2[1]</td></tr>";
		...
		for($z=1;$z<3;$z++)
		{
			if($z==1)
			{
				...
				$sql.=" and course_yearsem='$r1[0]' and quota_id='$r2[0]' and gender!='F' order by first_name";
			}
			else
			{	
				$sql="select student_id,first_name,last_name,course_yearsem,class_section_id from student_m where id is not null and archive='N'";
