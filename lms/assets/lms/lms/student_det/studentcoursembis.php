...
function reload()
{
	document.frm.action='studentcoursembis.php';
	document.frm.submit();
...
if($_POST['open'])
{
	...
	for($i=0;$i<sizeof($studentid);$i++)
	{
		...
		$sql5=execute(" select sub_sec from $tablename where acc_year='$temp_year_detalis' and stu_id='$studentid[$i]' and sub='$sess'");
...
<?
$rs_section=execute("select subject_id , subject_name from subject_m where course_id='$branch' and course_year_id='$sem' and elective='Y' and status=1");
