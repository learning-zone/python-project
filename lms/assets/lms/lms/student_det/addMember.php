...
while( list(,$Value) = each($sel) )
	{
		if($mType==1)
		{
			$qry = mysql_fetch_array(mysql_query("select * from student_m where id=$Value"));
			$s_id = $qry[student_id];				
		...
		if($mType==2)
		{
			$qry = mysql_fetch_array(mysql_query("select * from staff_det where id=$Value"));
			$s_id = $qry[slno];
		...
		if($mType==3)
		{
			$qry = mysql_fetch_array(mysql_query("select * from dept_no where dpt_id=$Value"));
			$s_id = $qry[dept_code];
			$sname = $qry[Dept];
			$qid=$qry[dpt_id];
		}
		...
		$result=mysql_query("select MAX(id),m_no from lib_membership_m");
