...
function ReloadMe()
{
    document.frm.action="pastoral.php";
	document.frm.submit();
}
</script>
<script language="javascript">
...
{
	?>
	<form method='post' action="pastoral.php" name="frm" >
 <table class='forumline' align='center' width="90%" >
   <tr>
   		<td Class="Head" colspan='7' align='center'>PASTORAL CARE</td>
   </tr>
	<tr height='30'>
		<td>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $_SESSION['branchname']; ?></td>
		<td><select name="branch" onChange="ReloadMe()">
        ...
        <select name="sem" onChange="ReloadMe()">
			<option value="0">------ Select ------</option>
			<?php
...
<?php
  ...
  $sno=1;
    ...
    $stDet=fetcharray(execute("SELECT admission_id,student_id,usn,first_name,last_name,course_yearsem,academic_year,class_section_id,course_admitted FROM `student_m` WHERE student_id='$r[student_id]' LIMIT 1"));

		?>
		<td   align='center' ><?=$sno?></td>
		        
       <!--<td align="center">&nbsp;&nbsp;
       <a href="javascript:OpenWind2('pastoral_edt.php?student_id=<?=$stDet['student_id']?>&class_section_id=<?=$stDet['class_section_id']?>&branch=<?=$stDet['course_admitted']?>&sem=<?=$stDet['course_yearsem']?>&studfname=<?=$stDet['first_name']?>&studlname=<?=$stDet['last_name']?>&a_year=<?=$stDet['academic_year']?>')" ><?=$r['student_id']?></a></td>-->
 
        <td align="center">&nbsp;&nbsp;
         <a href="javascript:void(0);" onClick ="OpenWind3('pastoral_edt.php?student_id=<?=$stDet['student_id']?>&class_section_id=<?=$stDet['class_section_id']?>&branch=<?=$stDet['course_admitted']?>&sem=<?=$stDet['course_yearsem']?>&studfname=<?=$stDet['first_name']?>&studlname=<?=$stDet['last_name']?>&a_year=<?=$stDet['academic_year']?>', 'OpenWind3',1000,700)"><?=$r['student_id']?></a></td>
