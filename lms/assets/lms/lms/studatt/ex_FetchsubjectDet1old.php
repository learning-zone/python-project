...
$sql21=execute("select a.class,a.section,a.sub,a.sub_type from all_teachers a,users b where  b.username='$user' and b.srid=a.home_teac group by a.sub");
...
if(rowcount($sql21)!=0)
{
	...
	while($r12=fetcharray($sql21))
	{
		...
		$rs_section=fetchrow(execute("select section_name,codename from class_section where id='$class_section_id1'"));
	...
	<tr height="25">
        ...
        <a href="javascript:OpenWind2('addattendace.php?subname=DAILY&branch=<?=$branch1?>&sem=<?=$sem1?>&class_section_id=<?=$class_section_id1?>&subject=<?=$r12[2]?>&sess=s')"><input type="button" name="update" value="Add Attendance"  class='bgbutton'></a>
        <?php
			}
			if($sql_id[0]==2)
			{
...
{	
    ...
    <tr >
        ...
        <td align="center" width="30" nowrap><a href="javascript:OpenWind2('addattendace.php?subname=<?=$subject_id_dis[0]?>&branch=<?=$branch?>&sem=<?=$sem?>&class_section_id=<?=$class_section?>&subject=<?=$subject?>&sess=s')"><input type="button" name="update" value="Add Attendance"  class='bgbutton'></a></td>
    </tr>
