...
function reload()
{
	document.frm.action='ModifyMarks.php';
	document.frm.submit();
}
    ...
    <td>&nbsp;<?php echo $_SESSION['branchname']; ?></td>
		<td>&nbsp;<select name="branch" onChange="reload()">
			<option value="0">------Select-----</option>
				<?php
					$sql="select course_id,coursename from course_m  where course_id='$branch1'";
  ...
  <tr>
   <td>&nbsp;<?php echo $_SESSION['semname']; ?></td>
		<td>&nbsp;<select name="sem" onChange="reload()">
			<option value='0'>-----Select----</option>
			<?php
				$rs=execute("SELECT a.year_name,a.year_id FROM course_year a,course_m b where a.head_id=b.head_id and b.course_id='$branch' and a.year_id='$sem1' ");
...
<?
$rs_section=execute("select * from class_section  where id='$class_section_id1'");
...
?>
  ...
  echo "<tr height='25'>
	...
	$sql111=execute("SELECT posi FROM exam_topers WHERE exam_id='$examname' and stud_id='$r1[id]'");
	if(rowcount($sql111)>0)
	{
	?><td nowrap align='center'>&nbsp;<a href= "javascript:OpenWind2('AddMarks1.php?course=<?=$branch?>&sem=<?=$sem?>&examid=<?=$examname?>&studentid=<?=$r1[id]?>&class_section_id=<?=$class_section_id?>&stundetname=<?=$r1[first_name]?>&student_id=<?=$r1[student_id]?>')">Modify MARKS			
</a>
    </td></tr><?php
	}
	else
	{
		?><td nowrap align='center'>&nbsp;<a href= "javascript:OpenWind2('AddMarks1.php?course=<?=$branch?>&sem=<?=$sem?>&examid=<?=$examname?>&studentid=<?=$r1[id]?>&class_section_id=<?=$class_section_id?>&stundetname=<?=$r1[first_name]?>&student_id=<?=$r1[student_id]?>')">ADD MARKS			
</a>
    </td></tr><?php
	}
$i++;  }
