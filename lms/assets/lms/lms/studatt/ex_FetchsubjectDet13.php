...
if(rowcount($sql21)!=0)
{
	...
	while($r12=fetcharray($sql21))
	{
		...
		$rs_section=fetchrow(execute("select section_name from class_section where id='$class_section_id1'"));
	
	?>
	<tr height="25">
		<td align="center" nowrap><?=$i?></td>
		<td align="center" nowrap><?=$yearname[0]?></td>
		<td align="center" nowrap><?=$rs_section[0]?></td>
        ...
        <a href="javascript:OpenWind2('addattendace.php?subname=DAILY&branch=<?=$branch1?>&sem=<?=$sem1?>&class_section_id=<?=$class_section_id1?>&subject=0&sess=b')"><input type="button" name="update" value="Add Attendance"  class='bgbutton'></a>
        <?php
			}
			if($sql_id[0]==2)
			{
