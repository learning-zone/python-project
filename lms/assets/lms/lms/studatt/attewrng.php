...
while($r21=fetcharray($sql21))
{
	...
	if($rs_section[0]!='' && $rs_section[1]!='')
	{
		...
		if($sql_id[0]==1 || $sql_id[0]==3)
		{
			...
			{ 
				...
				if($flag1==1)
				{
					...
					{
						$sysdate=$r2[0];
						$sql5=execute("select $rownameid from $tablename where att_date='$sysdate' and stu_id='$r1[id]' and sec='$class_section_id'  and $rownameid!=''");
						...
						$flag=1;
						$nr=fetchrow(execute("SELECT Short_name FROM `attendance_points` where order_id='$checkiddet[0]'"));
						$orderid=$checkiddet[0];
						if($nr[0])
						{
							$statuschek='checked';
							$naval=$nr[0];
			...
			<?php
	  echo $sql123="select b.id,b.student_id,b.first_name,b.last_name,b.admission_id from student_course a,student_m b where a.acc_year='$a_year' and a.stu_id=b.id and a.sub='$subject'  and b.archive='N'  and a.sub_sec='$class_section_id' group by a.stu_id order by b.first_name";
        $rs=execute($sql123);
		      $maxtot=0;
	  $maxper=0;
      ...
      while($r1=fetcharray($rs))
      {
		 /* echo "<tr><td colspan='5'> SELECT att_date FROM $tablename where (att_date between '$fdate' and '$todate') and sec='$class_section_id' and stu_id='$r1[0]' and ( mor=0 or mor=5) limit 1  </td></tr>";*/
		  $sql1val=fetchrow(execute("SELECT att_date FROM $tablename where (att_date between '$fdate' and '$todate') and sec='$class_section_id' and stu_id='$r1[0]' and ( mor=0 or mor=5) limit 1")); 
      if($sql1val[0])
	  $flag1=1;
	  else
	  $flag1=0;
