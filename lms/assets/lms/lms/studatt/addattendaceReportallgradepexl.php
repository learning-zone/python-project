...
$tablename="att_".$sem;
      ...
      <tr>
       ...
       <?php
            $sql1=execute("SELECT att_date FROM $tablename where (att_date between '$fdate' and '$todate') and sec='$class_section_id'   and mor=1 group by att_date order by att_date");
      ...
      { 
	   $sql1val=fetchrow(execute("SELECT att_date FROM $tablename where (att_date between '$fdate' and '$todate') and sec='$class_section_id'   and stu_id='$r1[0]' and (mor=1 or mor=2 or mor=3) limit 1")); 
      if($sql1val[0])
	  $flag1=1;
	  else
	  $flag1=0;
 ...
 $count=0;
            ...
            while($r2=fetcharray($sql1))
            {
                ...
                if($orderid==1 or $orderid==2 or $orderid==3)
                {
