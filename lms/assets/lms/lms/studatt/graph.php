...
function reload()
{
	document.frm.action='graph.php';
	document.frm.submit();
	
}
function reload1()
{
	document.frm.action='graph1.php';
	document.frm.submit();
...
?>		<table width="50%" align="center" class="forumline" border="1" cellspacing="0" cellpadding="0">
  <tr>
    <td colspan="2" align="center" class="head">Consolidated Report card</td>
    </tr>
...
?>
  ...
  echo "<tr>
	...
	?><a href= "javascript:OpenWind2('graph1.php?stuid=<?=$r1[student_id]?>&stuname=<?=$r1[first_name]?>&branch=<?=$branch?>&sem=<?=$sem?>&examid=<?=$examname?>&studentid=<?=$r1[id]?>&class_section_id=<?=$class_section_id?>&stundetname=<?=$r1[first_name]?>&admissionid=<?=$r1[admission_id]?>&student_id=<?=$r1[student_id]?>')">
	VIEW
