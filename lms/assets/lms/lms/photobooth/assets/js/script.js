$(document).ready(function(){
	...
	var template = '<a href="../student_images/temp/{src}" rel="cam" '
		+'style="background-image:url(../student_images/temp/{src})"></a>';

	/*----------------------------------
		Setting up the web camera
	----------------------------------*/

	var urlpth='upload.php?phid='+document.frm.phid.value+'&phpath='+document.frm.phpath.value;
	webcam.set_swf_url('assets/webcam/webcam.swf');
	webcam.set_api_url(urlpth);	// The upload script
	...
	webcam.set_hook('onComplete', function(msg){
		...
		if(msg.error){
			...
			if(msg.status)
			{
				self.opener.location.replace("../student_det/SearchStudent.php?phid="+msg.filename);
			}
			else
			{
				if(msg.mstatus)
				{
					self.opener.location.replace("../staff_det/update.php?phid="+msg.filename);
				}
				else
				{
