...
if(isset($_POST['update']))
{
	...
	for($i=0;$i<sizeof($cid);$i++)
	{
		...
		execute("update master_skills set skill='$skills', mark='$mark', posi='$priority' where id='$cid[$i]'");	
	}
		?>
		<Script language="JavaScript">
...
if(isset($_POST['save']))
{
	$sql2=execute("select * from master_skills where divi='$course' and class='$sem' and sub='$subject' and skill='$skills' and posi='$priority'");
	if(rowcount($sql2)>=1)
	{
		?>
		<Script language="JavaScript">
	...
	else
	{
		...
		execute("INSERT INTO master_skills (id, divi, class, sub, skill, posi,mark) VALUES (NULL, '$course', '$sem', '$subject', '$skills', '$priority','$mark')") or die(mysql_error());
		?>
		<Script language="JavaScript">
		alert("Updated successfully");
		</Script>
...
$sql3=execute("select id, skill, posi,mark from master_skills where divi='$course' and class='$sem' and sub='$subject'");
if(rowcount($sql3)>=1)
{	
	?>
<br>
...
<tr>
	...
	echo "<tr><td align='center'  nowrap><input type='checkbox' name='cid' value='$r6[0]'>
		...
		<td align='center' nowrap>";
		?>
<a href= "javascript:OpenWind2('sub_skills.php?master=<?php echo $r6[0]; ?>&&mastername=<?php echo $r6[1]; ?>')">
	ADD SUB SKILLS
