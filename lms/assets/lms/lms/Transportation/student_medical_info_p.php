...
if($gob)
{
...

	{
...

		{
			...
			execute("update student_health set checked=0 where student_id = '$student_id' ");
...

			{
				...
				execute("update student_health set checked=1 where student_id = '$student_id' and dname = '$ce'");
	...
	?>
		...
		echo "<META HTTP-EQUIV='Refresh' Content='0;URL=student_medical_info_p.php?student_id=<?=$student_id'>";
...

  {
	...
	document.frmMedicaldetail.submit();
            ...
            <?php
					...
					$fetch_name=fetcharray(execute("select first_name,last_name from student_m where id='$student_id'"));

					?>

			<td vAlign="top" align="Center" height="30" colspan=5 class=head>

			Student Medical Details for <?=$fetch_name[first_name]." ".$fetch_name[last_name]?></td>
...
<tr height="25">
   ...
   <td class="formtable" >Please tick in the appropriate box if you have any history of the following:</td><td colspan="4"></td>
...
$sql=execute("select * from student_med_frm order by id");
...

{
	...
	$qry=fetchrow(execute("select checked from student_health where student_id='$student_id' and dname=$r1[id]"));
  ...
  ?>
    ...
    <td>&nbsp;<input type="radio" name="type5"  value="no"  <?=$check9?>>No</td>
		...
		<div>
            ...
            <a href="javascript:OpenWind8('print_medical_form.php?student_id=<?=$student_id?>')" >
