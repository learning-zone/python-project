...

{
	...
	$sql=execute("select * from trans_point_details where route_id=$route and point_id=$point");
	if(rowcount($sql)>=1)
	{
	...
	else
	{
		...
		$maxid=fetchrow(execute("select max(id) from trans_point_details where route_id='$route' and point_id='$point' and pick_t='$p_time'"));	
	 	...
	 	while($r=fetcharray($rs123))
	 	{
			...
			//echo "update point_details set drop_time$r[0]='$pontval' where id='$maxid[0]'";
$sql_upd=execute("update trans_point_details set drop_time$r[0]='$pontval' where id='$maxid[0]'"); 
		...
		//echo "<div>Details inserted...!!</div><br>";
...
while($frt=fetcharray($fetch_qry))
{
    ...
    <tr>
	 <?
            ...
            <td align="center">
           <select name="<?=$row[id]?>route1">
             <option value="">---- Select ----</option>
          	 <?php
					
					$sqlR=execute("SELECT * FROM `trans_route_master` ORDER BY id");
			...
			<td align="center">
            <select name="pickpoint1" onchange='reload()'>
			...
			<?php
				...
				if($pickpoint1==$ss[0])

					{
						...
						<option value='<?php echo $frt[id] ?>' selected><?php echo $ss[0] ?></option>
...

					{
						...
						<option value='<?php echo $frt[id] ?>'><?php echo $ss[0] ?></option>
...
?>
  ...
  <br>
 <div align="center"><input Type="Button" class=bgbutton Value="Delete" onClick="deldata()"><Input type="submit" Name="Modify" value="Modify" LANGUAGE=javascript onClick="return Modify_onclick()" class='bgbutton'></div>
  </form>
  <?
}
?>
